# Tutorial for the 1D-event-model at the catchment scale

The tutorial simulates 448256 (544 x 824) grid cells.

1D indicates indicates that vertical processes and lateral processes are considered.

- `parameters`: Contains spatially distributed model parameters as .flt files
...
- `oneD_event.sh`: Runs the 1D-model setup using the numpy backend
- `oneD_event_parallel.sh`: Runs the 1D-model setup using the parallel numpy backend
- `oneD_event_gpu.sh`: Runs the 1D-model setup on a NVIDIA GPU device using the jax backend
