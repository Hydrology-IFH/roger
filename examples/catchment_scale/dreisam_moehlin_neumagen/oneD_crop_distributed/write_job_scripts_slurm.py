...
def main():
    ...
    for stress_test_meteo in stress_tests_meteo:
        if stress_test_meteo == "base":
            ...
            scenario_flags.append('--stress-test-meteo %s --soil-compaction --irrigation\n' % (stress_test_meteo))
            scenario_flags.append('--stress-test-meteo %s --soil-compaction --yellow-mustard\n' % (stress_test_meteo))
            ...
            for magnitude in stress_test_meteo_magnitudes:
                for duration in stress_test_meteo_durations:
                    if magnitude == 0 and duration == 0:
                        ...
                        scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s --soil-compaction\n' % (stress_test_meteo, magnitude, duration))
                        scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s\n' % (stress_test_meteo, magnitude, duration))
                        scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s --soil-compaction --irrigation\n' % (stress_test_meteo, magnitude, duration))
                        scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s --soil-compaction --yellow-mustard\n' % (stress_test_meteo, magnitude, duration))
    ...
    for scenario_flag, script_name in zip(scenario_flags[:2], script_names[:2]):
        ...
        lines.append('python oneD_crop.py -b jax -d cpu %s -td "${TMPDIR}"\n' % (scenario_flag))
