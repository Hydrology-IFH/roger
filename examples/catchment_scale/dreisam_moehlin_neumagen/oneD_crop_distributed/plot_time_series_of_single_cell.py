...
def main():
    ...
    for year in years:
        if year == 2018:
            ...
            base_recharge_ts = base_recharge[:, x, y]
            scenario_recharge_ts = scenario_recharge[:, x, y]
        ...
        ds_base.close()
        date_time = pd.date_range(start=f"{year}-01-01", end=f"{year}-12-31", freq="D")
        base_ground_cover_ts = base_ground_cover[:, x, y]
        ...
        ds_base.close()
        date_time = pd.date_range(start=f"{year}-01-01", end=f"{year}-12-31", freq="D")
        base_z_root_ts = base_z_root[:, x, y]
