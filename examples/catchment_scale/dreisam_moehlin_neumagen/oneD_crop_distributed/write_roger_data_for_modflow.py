                    ...
                    if os.path.exists(diag_file):
                        with h5netcdf.File(diag_file, "r", decode_vlen_strings=False) as df:
                            ...
                            #                 v.attrs.update(long_name=var_object.attrs["long_name"], units=var_object.attrs["units"])
                                ...
                                output_file = base_path_output / f"root_depth_{meteo_stress_test}_{irrigation_scenario}_{catch_crop_scenario}_{soil_compaction_scenario}_year{year}.nc"
                                if not os.path.exists(output_file):
                                    with h5netcdf.File(output_file, "w", decode_vlen_strings=False) as f:
                                        f.attrs.update(
                                            date_created=datetime.datetime.today().isoformat(),
                                ...
                                output_file = base_path_output / f"ground_cover_{meteo_stress_test}_{irrigation_scenario}_{catch_crop_scenario}_{soil_compaction_scenario}_year{year}.nc"
                                if not os.path.exists(output_file):
                                    with h5netcdf.File(output_file, "w", decode_vlen_strings=False) as f:
                                        f.attrs.update(
                                            date_created=datetime.datetime.today().isoformat(),
