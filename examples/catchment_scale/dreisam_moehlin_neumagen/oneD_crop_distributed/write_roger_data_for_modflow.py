    ...
    if os.path.exists(diag_file):
        with h5netcdf.File(diag_file, "r", decode_vlen_strings=False) as df:
            ...
            for year in years:
                ...
                output_file = base_path_output / f"actual_evapotranspiration_{stress_test_meteo}-magnitude{stress_test_meteo_magnitude}-duration{stress_test_meteo_duration}_{irrigation}_{yellow_mustard}_{soil_compaction}{_grain_corn_only}_year{year}.nc"
                files_to_compress.append(output_file)
                if not os.path.exists(output_file):
                    with h5netcdf.File(output_file, "w", decode_vlen_strings=False) as f:
                        f.attrs.update(
                            date_created=datetime.datetime.today().isoformat(),
                            title=f"RoGeR actual evapotranspiration simulations of the Dreisam-Moehlin-Neumagen catchment - Year {year}",
                            institution="University of Freiburg, Chair of Hydrology",
                            references="",
                            comment="First timestep (t=0) contains initial values. Simulations start are written from second timestep (t=1) to last timestep (t=N).",
                            model_structure="ONED model with free drainage and explicit crop growth dynamics",
                        ...
                        with h5netcdf.File(diag_file, "r", decode_vlen_strings=False) as df:
                            time_indices = onp.where(date_time.year == year)[0]
                            v = f.create_variable("actual_evapotranspiration", ("Time", "y", "x"), float, compression="gzip", compression_opts=1)
                            var_object = df.variables.get("aet")
                            v[:, :, :] = var_object[time_indices, :, :]
                            v.attrs.update(long_name=var_object.attrs["long_name"], units=var_object.attrs["units"], grid_mapping="spatial_ref")
