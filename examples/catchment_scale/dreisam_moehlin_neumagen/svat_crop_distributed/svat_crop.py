...
def main(tmp_dir):
    ...
    class SVATCROPSetup(RogerSetup):
        ...
        def set_settings(self, state):
            ...
            station_ids = onp.unique(self._read_var_from_nc_xr("station_id", self._base_path, "parameters.nc"))
        ...
        def set_topography(self, state):
            ...
            # catchment mask (bool)
            maskCatch = self._read_var_from_nc_xr("maskCatch", self._base_path, "parameters.nc")
            vs.maskCatch = update(
                vs.maskCatch,
                at[2:-2, 2:-2],
                maskCatch == 1,
        ...
        def set_parameters_setup(self, state):
            ...
            vs.station_id = update(
                vs.station_id,
                at[2:-2, 2:-2],
                self._read_var_from_nc_xr("station_id", self._base_path, "parameters.nc")
            )
            ...
            vs.crop_type = update(
                vs.crop_type,
                at[2:-2, 2:-2, 1],
                self._read_var_from_nc("summer_crops", self._input_dir, "crop_rotations_2000-2023.nc")[0, :, :],
            )
            vs.crop_type = update(
                vs.crop_type,
                at[2:-2, 2:-2, 2],
                self._read_var_from_nc("winter_crops", self._input_dir, "crop_rotations_2000-2023.nc")[0, :, :],
            )
            mask1 = npx.isin(vs.crop_type[:, :, 2], [574, 595, 597])
            vs.crop_type = update(
                vs.crop_type,
                at[2:-2, 2:-2, 0],
        ...
        def set_forcing(self, state):
            ...
            if (vs.year[1] != vs.year[0]) & (vs.itt > 1):
                ...
                vs.crop_type = update(
                    vs.crop_type,
                    at[2:-2, 2:-2, 1],
                    self._read_var_from_nc("summer_crops", self._input_dir, "crop_rotations_2000-2023.nc")[vs.itt_cr, :, :],
                )
                vs.crop_type = update(
                    vs.crop_type,
                    at[2:-2, 2:-2, 2],
                    self._read_var_from_nc("winter_crops", self._input_dir, "crop_rotations_2000-2023.nc")[vs.itt_cr, :, :],
                )
    ...
    write_forcing_distributed(model._input_dir, enable_crop_phenology=True)
