...
def main():
    ...
    class SVATSetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            vs.dmpv = update(vs.dmpv, at[2:-2, 2:-2], [236, 259])
            vs.lmpv = update(vs.lmpv, at[2:-2, 2:-2], [933, 202])
            vs.theta_ac = update(vs.theta_ac, at[2:-2, 2:-2], [0.186, 0.098])
            vs.theta_ufc = update(vs.theta_ufc, at[2:-2, 2:-2], [0.108, 0.06])
            vs.theta_pwp = update(vs.theta_pwp, at[2:-2, 2:-2], [0.24, 0.23])
            vs.ks = update(vs.ks, at[2:-2, 2:-2], [33.8, 145.1])
