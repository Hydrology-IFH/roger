...
file = base_path / "input" / "stress_tests_meteo" / "ta_autumn_stress_magnitude.csv"
df_ta_autumn_stress_magnitude = pd.read_csv(file, sep=";", skiprows=1)
file = base_path / "input" / "stress_tests_meteo" / "ta_winter_stress_magnitude.csv"
df_ta_winter_stress_magnitude = pd.read_csv(file, sep=";", skiprows=1)
...
file = base_path / "input" / "stress_tests_meteo" / "prec_autumn_stress_magnitude.csv"
df_prec_autumn_stress_magnitude = pd.read_csv(file, sep=";", skiprows=1)
file = base_path / "input" / "stress_tests_meteo" / "prec_winter_stress_magnitude.csv"
...
file = base_path / "input" / "stress_tests_meteo" / "pet_autumn_stress_magnitude.csv"
df_pet_autumn_stress_magnitude = pd.read_csv(file, sep=";", skiprows=1)
file = base_path / "input" / "stress_tests_meteo" / "pet_winter_stress_magnitude.csv"
df_pet_winter_stress_magnitude = pd.read_csv(file, sep=";", skiprows=1)

meteo_stations_name = df_ta_spring_stress_magnitude["station"].values.astype(str)
meteo_stations = df_ta_spring_stress_magnitude["ID"].values
# make dictionary to link station ID to station name
dict_meteo_stations = dict(zip(meteo_stations, meteo_stations_name))
durations = [0, 2, 3]
magnitudes = [0, 1, 2]

for station in meteo_stations:
    meteo_station_name = dict_meteo_stations[station]
    prec_10mins, df_pet, df_ta = read_meteo(meteo_path / str(meteo_station_name).lower())
    ...
    for duration in durations:
        for magnitude in magnitudes:
            if magnitude == 0 and duration == 3:
                path_to_dir = base_path  / "input" / "stress_tests_meteo" / "spring-drought" / f"duration{duration}_magnitude{magnitude}" / f"{meteo_station_name.lower()}" 
                ...
                path_to_dir = base_path  / "input" / "stress_tests_meteo" / "summer-drought" / f"duration{duration}_magnitude{magnitude}" / f"{meteo_station_name.lower()}" 
                if not os.path.exists(path_to_dir):
                    os.makedirs(path_to_dir)  
                ...
                path_to_dir = base_path  / "input" / "stress_tests_meteo" / "long-term" / f"duration{duration}_magnitude{magnitude}" / f"{meteo_station_name.lower()}" 
                if not os.path.exists(path_to_dir):
                    os.makedirs(path_to_dir)  
                ...
                prec_magnitude_autumn = df_prec_autumn_stress_magnitude.loc[df_prec_autumn_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                ta_magnitude_autumn = df_ta_autumn_stress_magnitude.loc[df_ta_autumn_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                pet_magnitude_autumn = df_pet_autumn_stress_magnitude.loc[df_pet_autumn_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                prec_magnitude_winter = df_prec_winter_stress_magnitude.loc[df_prec_winter_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                ta_magnitude_winter = df_ta_winter_stress_magnitude.loc[df_ta_winter_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                pet_magnitude_winter = df_pet_winter_stress_magnitude.loc[df_pet_winter_stress_magnitude["ID"] == station, f"magnitude{magnitude}"].values[0]
                # select only spring periods and modify them according to the stress magnitude
                cond = (prec_10mins.index.month.isin([3, 4, 5]))
                prec_10mins.loc[cond, "PREC"] = prec_10mins.loc[cond, "PREC"].values * (1 + (prec_magnitude_spring / 100))
                cond = (df_ta.index.month.isin([3, 4, 5]))
                df_ta.loc[cond, "TA":] = df_ta.loc[cond, "TA":].values + ta_magnitude_spring
                df_pet.loc[cond, "PET"] = df_pet.loc[cond, "PET"].values * (1 + (pet_magnitude_spring / 100))
                # select only summer periods and modify them according to the stress magnitude
                cond = (prec_10mins.index.month.isin([6, 7, 8]))
                prec_10mins.loc[cond, "PREC"] = prec_10mins.loc[cond, "PREC"].values * (1 + (prec_magnitude_summer / 100))
                cond = (df_ta.index.month.isin([6, 7, 8]))
                df_ta.loc[cond, "TA":] = df_ta.loc[cond, "TA":].values + ta_magnitude_summer
                df_pet.loc[cond, "PET"] = df_pet.loc[cond, "PET"].values * (1 + (pet_magnitude_summer / 100))
                # select only autumn periods and modify them according to the stress magnitude
                cond = (prec_10mins.index.month.isin([9, 10, 11]))
                prec_10mins.loc[cond, "PREC"] = prec_10mins.loc[cond, "PREC"].values * (1 + (prec_magnitude_autumn / 100))
                cond = (df_ta.index.month.isin([9, 10, 11]))
                df_ta.loc[cond, "TA":] = df_ta.loc[cond, "TA":].values + ta_magnitude_autumn
                df_pet.loc[cond, "PET"] = df_pet.loc[cond, "PET"].values * (1 + (pet_magnitude_autumn / 100))
                # select only winter periods and modify them according to the stress magnitude
                cond = (prec_10mins.index.month.isin([12, 1, 2]))
                prec_10mins.loc[cond, "PREC"] = prec_10mins.loc[cond, "PREC"].values * (1 + (prec_magnitude_winter / 100))
                cond = (df_ta.index.month.isin([12, 1, 2]))
                df_ta.loc[cond, "TA":] = df_ta.loc[cond, "TA":].values + ta_magnitude_winter
                df_pet.loc[cond, "PET"] = df_pet.loc[cond, "PET"].values * (1 + (pet_magnitude_winter / 100))
                ...
                path_to_dir = base_path  / "input" / "stress_tests_meteo" / "spring-summer-wet" / f"{meteo_station_name.lower()}" 
                if not os.path.exists(path_to_dir):
                    os.makedirs(path_to_dir)  
