...
def main():
    ...
    for location in locations:
        ...
        for irrigation_demand_scenario in irrigation_demand_scenarios:
            if irrigation_demand_scenario == "20-ufc":
                c_irr = 0.2
            elif irrigation_demand_scenario == "30-ufc":
                c_irr = 0.30
            elif irrigation_demand_scenario == "50-ufc":
                c_irr = 0.50
            if not os.path.exists(str(base_path / "output" / dir_name / irrigation_demand_scenario / location)):
            ...
            for crop_rotation_scenario in crop_rotation_scenarios:
                ...
                if os.path.exists(roger_file):
                    ...
                    dir_csv_files = base_path / "output" / dir_name / irrigation_demand_scenario / crop_rotation_scenario / location
                    if not os.path.exists(dir_csv_files):
                        os.makedirs(dir_csv_files)

                    ll_area_weighted = []
                    for x, soil_type in enumerate(soil_types):
                        dir_csv_files = base_path / "output" / dir_name / irrigation_demand_scenario / crop_rotation_scenario / location / soil_type
                        if not os.path.exists(dir_csv_files):
                            os.makedirs(dir_csv_files)
                        ...
                        if irrigation_demand_scenario in ["15-ufc", "30-ufc", "50-ufc"]:
                        ...
                        df_simulation.columns =[["[mm/day]", "[mm/day]", "[mm/day]", "[-]", "[-]", "[mm]", "[-]", "[-]", "[-]", "[mm]", "[%]", "[degC]", "[degC]", "[day]", "[mm/day]", "[mm/day]", "[mm/day]", "", "", "[mm/day]"],
                                                ["precip", "pet", "pt", "photosynthesis_index", "canopy_cover", "z_root", "theta_fc", "theta_irrig", "theta_rz", "irrigation_demand", "root_ventilation", "ta", "ta_max", "heat_stress", "transp", "evap_soil", "perc", "lu_id", "crop_type", "inf"]]
                    ...
                    dir_csv_files = base_path / "output" / dir_name / irrigation_demand_scenario / location / crop_rotation_scenario / "area_weighted"
                    if not os.path.exists(dir_csv_files):
                        os.makedirs(dir_csv_files)
                    df_simulation.to_csv(
                            dir_csv_files / "simulation.csv", sep=";"
