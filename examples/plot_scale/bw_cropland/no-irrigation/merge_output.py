                    ...
                    if diag_files:
                        with h5netcdf.File(output_hm_file, "w", decode_vlen_strings=False) as f:
                            f.attrs.update(
                                ...
                                title=f"RoGeR simulations using DWD station {location} with the {stress_test_meteo} (duration {duration} and magnitude {magnitude}) climate scenario as input and {crop_rotation_scenario1} as crop rotation and no irrigation.",
