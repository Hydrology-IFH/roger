...
def main():
    ...
    agricultural_management_scenarios = ["no-irrigation", "no-irrigation_soil-compaction", "irrigation_soil-compaction", "irrigation"]

    # merge model output into single file
    for scenario in agricultural_management_scenarios:
                    ...
                    if os.path.exists(str(base_path / "output" / dir_name / scenario)):
                        for crop_rotation_scenario in crop_rotation_scenarios:
                            ...
                            if scenario == "no-irrigation":
                                path = str(base_path / "output" / dir_name / scenario / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}_no-irrigation.*.nc")
                                output_file = base_path / "output" / dir_name / scenario / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}.nc"
                            elif scenario == "no-irrigation_soil-compaction":
                                path = str(base_path / "output" / dir_name / scenario / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}_no-irrigation_soil-compaction.*.nc")
                                output_file = base_path / "output" / dir_name / scenario / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}.nc"
                            elif scenario == "irrigation_soil-compaction":
                                path = str(base_path / "output" / dir_name / scenario / "crop-specific" / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}_crop-specific-irrigation_soil-compaction.*.nc")
                                output_file = base_path / "output" / dir_name / scenario / "crop-specific" / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}.nc"
                            elif scenario == "irrigation":
                                path = str(base_path / "output" / dir_name / scenario / "crop-specific" / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}_crop-specific-irrigation.*.nc")
                                output_file = base_path / "output" / dir_name / scenario / "crop-specific" / f"SVATCROPNITRATE_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}.nc"
                                ...
                                if diag_files:
                                    with h5netcdf.File(output_file, "w", decode_vlen_strings=False) as f:
                                        f.attrs.update(
                                            ...
                                            title=f"RoGeR-SAS-Nitrate simulations using DWD station {location} as input and {crop_rotation_scenario1} as crop rotation",
