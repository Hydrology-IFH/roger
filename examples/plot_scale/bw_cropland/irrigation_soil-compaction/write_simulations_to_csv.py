    ...
    for location in locations:
        ...
        for stress_test_meteo in stress_tests_meteo:
            ...
            for duration_magnitude in durations_magnitudes:
                ...
                if os.path.exists(str(base_path / "output" / dir_name / stress_test_meteo / f"duration{duration}_magnitude{magnitude}" / location)):
                    for crop_rotation_scenario in crop_rotation_scenarios:
                        roger_file = (
                            base_path
                            / "output" 
                            / dir_name 
                            / f"SVATCROP_{stress_test_meteo}-duration{duration}-magnitude{magnitude}_{location}_{crop_rotation_scenario}.nc"
                        )
                        if os.path.exists(roger_file):
                            ...
                            dir_csv_files = base_path / "output" / dir_name / stress_test_meteo / f"duration{duration}_magnitude{magnitude}" / location / crop_rotation_scenario
                            if not os.path.exists(dir_csv_files):
                                os.makedirs(dir_csv_files)
                            ll_area_weighted = []
                            for x, soil_type in enumerate(soil_types):
                                dir_csv_files = base_path / "output" / dir_name / stress_test_meteo / f"duration{duration}_magnitude{magnitude}" / location / crop_rotation_scenario / soil_type
                                if not os.path.exists(dir_csv_files):
                                    os.makedirs(dir_csv_files)
