        ...
        for irrigation_scenario in irrigation_scenarios:
            if os.path.exists(str(base_path / "output" / dir_name / irrigation_scenario)):
                for crop_rotation_scenario in crop_rotation_scenarios:
                    ...
                    path = str(base_path / "output" / dir_name / irrigation_scenario / f"SVATCROP_{location}_{crop_rotation_scenario}.*.nc")
                    output_hm_file = base_path / "output" / dir_name / irrigation_scenario / f"SVATCROP_{location}_{crop_rotation_scenario}.nc"
                        ...
                        if diag_files:
                            with h5netcdf.File(output_hm_file, "w", decode_vlen_strings=False) as f:
                                f.attrs.update(
                                    ...
                                    title=f"RoGeR simulations using DWD station {location} as input and {crop_rotation_scenario1} as crop rotation",
