    ...
    for land_cover_scenario in land_cover_scenarios:
        for climate_scenario in climate_scenarios:
            for period in periods:
                ...
                output_hm_file = base_path.parent / "output" / "svat" / f"SVAT_{location}_{land_cover_scenario}_{climate_scenario}_{period}.nc"
...
for location in locations:
    for land_cover_scenario in land_cover_scenarios:
        ...
        output_hm_file = base_path.parent / "output" / "svat" / f"SVAT_{location}_{land_cover_scenario}_observed_2015-2021.nc"
        if not os.path.exists(output_hm_file):
            diag_files = glob.glob(path)
            with h5netcdf.File(output_hm_file, 'w', decode_vlen_strings=False) as f:
                f.attrs.update(
...
for location in locations:
    for climate_scenario in climate_scenarios:
        for period in periods:
            ...
            output_hm_file = base_path.parent / "output" / "svat" / f"SVAT_{location}_grass_{climate_scenario}_{period}.nc"
            if not os.path.exists(output_hm_file):
                diag_files = glob.glob(path)
                with h5netcdf.File(output_hm_file, 'w', decode_vlen_strings=False) as f:
                    f.attrs.update(
...
for location in locations:
    path = str(base_path.parent / "output" / "svat" / f"SVAT_{location}_grass_observed_2016-2021.*.nc")
    output_hm_file = base_path.parent / "output" / "svat" / f"SVAT_{location}_grass_observed_2016-2021.nc"
    if not os.path.exists(output_hm_file):
        diag_files = glob.glob(path)
        with h5netcdf.File(output_hm_file, 'w', decode_vlen_strings=False) as f:
            f.attrs.update(
