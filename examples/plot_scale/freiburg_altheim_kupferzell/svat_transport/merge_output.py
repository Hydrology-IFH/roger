...
#                 output_tm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_{climate_scenario}_{period}.nc"
...
#         output_tm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_observed_2015-2021.nc"
#         diag_files = glob.glob(path)
#         with h5netcdf.File(output_tm_file, 'w', decode_vlen_strings=False) as f:
#             f.attrs.update(
#                 date_created=datetime.datetime.today().isoformat(),
...
for location in locations:
    for climate_scenario in climate_scenarios:
        for period in periods:
            ...
            output_tm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_grass_{climate_scenario}_{period}.nc"
            if not os.path.exists(output_tm_file):
                diag_files = glob.glob(path)
                with h5netcdf.File(output_tm_file, 'w', decode_vlen_strings=False) as f:
                    f.attrs.update(
