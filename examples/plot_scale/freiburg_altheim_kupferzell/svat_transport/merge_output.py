    ...
    for land_cover_scenario in land_cover_scenarios:
        for climate_scenario in climate_scenarios:
            for period in periods:
                path = str(base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_{climate_scenario}_{period}.*.nc")
                states_hm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_{climate_scenario}_{period}.nc"
...
for location in locations:
    for land_cover_scenario in land_cover_scenarios:
        path = str(base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_observed_2015-2021.*.nc")
        states_hm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_{land_cover_scenario}_observed_2015-2021.nc"
        diag_files = glob.glob(path)
        with h5netcdf.File(states_hm_file, 'w', decode_vlen_strings=False) as f:
            f.attrs.update(
                date_created=datetime.datetime.today().isoformat(),
...
for location in locations:
    for climate_scenario in climate_scenarios:
        for period in periods:
            path = str(base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_grass_{climate_scenario}_{period}.*.nc")
            states_hm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_grass_{climate_scenario}_{period}.nc"
            diag_files = glob.glob(path)
            with h5netcdf.File(states_hm_file, 'w', decode_vlen_strings=False) as f:
                f.attrs.update(
                    date_created=datetime.datetime.today().isoformat(),
...
for location in locations:
    path = str(base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_grass_observed_2016-2021.*.nc")
    states_hm_file = base_path.parent / "output" / "svat_transport" / f"SVATTRANSPORT_{location}_grass_observed_2016-2021.nc"
    diag_files = glob.glob(path)
    with h5netcdf.File(states_hm_file, 'w', decode_vlen_strings=False) as f:
        f.attrs.update(
            date_created=datetime.datetime.today().isoformat(),
