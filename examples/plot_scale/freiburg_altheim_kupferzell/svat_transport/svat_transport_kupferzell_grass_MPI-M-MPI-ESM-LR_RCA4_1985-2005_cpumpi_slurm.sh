...
#SBATCH --output=svat_transport_kupferzell_grass_MPI-M-MPI-ESM-LR_RCA4_1985-2005
#SBATCH --error=svat_transport_kupferzell_grass_MPI-M-MPI-ESM-LR_RCA4_1985-2005
#SBATCH --export=ALL
...
# Compares hashes
checksum_gws=$(shasum -a 256 /home/fr/fr_fr/fr_rs1092/roger/examples/plot_scale/freiburg_altheim_kupferzell/output/svat/SVAT_kupferzell_grass_MPI-M-MPI-ESM-LR_RCA4_1985-2005.nc | cut -f 1 -d " ")
checksum_ssd=0a
cp /home/fr/fr_fr/fr_rs1092/roger/examples/plot_scale/freiburg_altheim_kupferzell/output/svat/SVAT_kupferzell_grass_MPI-M-MPI-ESM-LR_RCA4_1985-2005.nc "${TMPDIR}"
# Wait for termination of moving files
while [ "${checksum_gws}" != "${checksum_ssd}" ]; do
    sleep 10
    checksum_ssd=$(shasum -a 256 "${TMPDIR}"/SVAT_kupferzell_grass_MPI-M-MPI-ESM-LR_RCA4_1985-2005.nc | cut -f 1 -d " ")
done
echo "Copying was successful"
 
mpirun --bind-to core --map-by core -report-bindings python svat_transport.py -b jax -d cpu -n 4 1 --location kupferzell --land-cover-scenario grass --climate-scenario MPI-M-MPI-ESM-LR_RCA4 --period 1985-2005 -td "${TMPDIR}"
