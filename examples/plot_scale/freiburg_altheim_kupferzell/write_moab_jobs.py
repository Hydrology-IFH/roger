...
def main():
        ...
        for land_cover_scenario in land_cover_scenarios:
            for climate_scenario in climate_scenarios:
                for period in periods:
                    ...
                    lines.append(' \n')
                    lines.append('# Move fluxes and states from global workspace to local SSD\n')
                    file_nc = 'SVAT_%s_%s_%s_%s.nc' % (location, land_cover_scenario, climate_scenario, period)
                    lines.append('mv %s/freiburg_altheim_kupferzell/output/svat/%s "${TMPDIR}"/%s\n' % (output_path_ws.as_posix(), file_nc, file_nc))
                    lines.append('# Wait for termination of moving files\n')
                    lines.append(f'checksum_gws=$(shasum -a 256 {output_path_ws.as_posix()}/freiburg_altheim_kupferzell/output/svat/{file_nc} | cut -f 1 -d " ")\n')
                    ...
                    lines.append('done\n')
                    lines.append(' \n')
                    lines.append('python svat_crop_transport.py -b jax -d gpu --location %s --land-cover-scenario %s --climate-scenario %s --period %s -td "${TMPDIR}"\n' % (location, land_cover_scenario, climate_scenario, period))
                    lines.append('# Move output from local SSD to global workspace\n')
                    lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
                    lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
                    lines.append('mv "${TMPDIR}"/*.nc %s\n' % (output_path_ws.as_posix()))
    ...
    for location in locations:
        for climate_scenario in climate_scenarios:
            for period in periods:
                ...
                lines.append('done\n')
                lines.append(' \n')
                lines.append('python svat_crop_transport.py -b jax -d gpu --location %s --land-cover-scenario grass --climate-scenario %s --period %s -td "${TMPDIR}"\n' % (location, climate_scenario, period))
                lines.append('# Move output from local SSD to global workspace\n')
                lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
                lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
                lines.append('mv "${TMPDIR}"/*.nc %s\n' % (output_path_ws.as_posix()))
    ...
    for location in locations:
        ...
        lines.append('done\n')
        lines.append(' \n')
        lines.append('python svat_crop_transport.py -b jax -d gpu --location %s --land-cover-scenario grass --climate-scenario observed --period 2016-2021 -td "${TMPDIR}"\n' % (location))
        lines.append('# Move output from local SSD to global workspace\n')
        lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
        lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
        lines.append('mv "${TMPDIR}"/*.nc %s\n' % (output_path_ws.as_posix()))
