        ...
        for land_cover_scenario in land_cover_scenarios:
            for climate_scenario in climate_scenarios:
                for period in periods:
                    ...
                    lines.append(' \n')
                    lines.append('python svat_crop.py -b jax -d cpu --land-cover-scenario %s --climate-scenario %s --period %s -td "${TMPDIR}"\n' % (land_cover_scenario, climate_scenario, period))
    ...
    for location in locations:
        for climate_scenario in climate_scenarios:
            for period in periods:
                ...
                lines.append(f'cd {base_path_binac}\n')
                lines.append(' \n')
                lines.append('python svat.py -b jax -d cpu --land-cover-scenario grass --climate-scenario %s --period %s -td "${TMPDIR}"\n' % (climate_scenario, period))
                lines.append('# Move output from local SSD to global workspace\n')
                lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
                lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
                lines.append('mv "${TMPDIR}"/*.nc %s\n' % (output_path_ws.as_posix()))
