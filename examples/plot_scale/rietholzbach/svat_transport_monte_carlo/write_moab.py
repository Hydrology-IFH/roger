...
for tm in transport_models:
    ...
    lines.append('mpirun --bind-to core --map-by core -report-bindings python svat_transport.py -b numpy -d cpu -n 25 1 -tms %s -td "${TMPDIR}"\n' % (tms))
    lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
    # lines.append(f'mpirun --bind-to core --map-by core -report-bindings python svat_transport.py -b numpy -d cpu -n 50 1 -tms {tms} -td {output_path_ws.as_posix()}\n')
