    ...
    class SVATTRANSPORTSetup(RogerSetup):
        ...
        def _get_nitt(self, path_dir, file):
            ...
            with h5netcdf.File(nc_file, "r", decode_vlen_strings=False) as infile:
                ...
                return len(npx.array(var_obj)) + 1

        def _get_runlen(self, path_dir, file):
            nc_file = path_dir / file
            with h5netcdf.File(nc_file, "r", decode_vlen_strings=False) as infile:
                var_obj = infile.variables['Time']
                return len(npx.array(var_obj)) * 60 * 60 * 24

        def _get_nx(self, path_dir, file):
            nc_file = path_dir / file
            with h5netcdf.File(nc_file, "r", decode_vlen_strings=False) as infile:
                var_obj = infile.variables['x']
                return len(npx.array(var_obj))
        ...
        def set_settings(self, state):
            ...
            settings.nx, settings.ny = self._get_nx(self._base_path, 'states_hm_sensitivity.nc'), 1
        ...
        def set_parameters_setup(self, state):
            ...
            vs.S_pwp_rz = update(vs.S_pwp_rz, at[2:-2, 2:-2], self._read_var_from_nc("S_pwp_rz", self._base_path, 'states_hm_sensitivity.nc')[:, :, vs.itt])
            vs.S_pwp_ss = update(vs.S_pwp_ss, at[2:-2, 2:-2], self._read_var_from_nc("S_pwp_ss", self._base_path, 'states_hm_sensitivity.nc')[:, :, vs.itt])
            vs.S_sat_rz = update(vs.S_sat_rz, at[2:-2, 2:-2], self._read_var_from_nc("S_sat_rz", self._base_path, 'states_hm_sensitivity.nc')[:, :, vs.itt])
            vs.S_sat_ss = update(vs.S_sat_ss, at[2:-2, 2:-2], self._read_var_from_nc("S_sat_ss", self._base_path, 'states_hm_sensitivity.nc')[:, :, vs.itt])
