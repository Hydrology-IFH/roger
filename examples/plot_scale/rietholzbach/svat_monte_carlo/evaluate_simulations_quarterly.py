...
def main(tmp_dir):
    ...
    file = base_path_figs / "params_metrics_quarterly.txt"
            ...
            for nrow in range(nx * ny):
                for sc, sc1 in zip([0, 1, 2, 3], ["", "dry", "normal", "wet"]):
                    ...
                    if var_sim in ["dS"]:
                        ...
                        df_eval_quarterly = df_eval.resample("3ME").mean()
                        df_eval = df_eval_quarterly.dropna()
                        ...
                        df_eval_quarterly = df_eval.resample("3ME").sum()
                        df_eval = df_eval_quarterly.dropna()
                        obs_vals = df_eval.loc[:, "obs"].values.astype(float)
                        sim_vals = df_eval.loc[:, "sim"].values.astype(float)
                        key_kge = "KGE_" + var_sim + f"{sc1}"
