...
@click.option("-ns", "--nsamples", type=int, default=15000)
    ...
    class SVATSetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            vs.c1_mak = update(
                ...
                at[2:-2, 2:-2],
                npx.array(
                    random_uniform(self._bounds["c1_mak"][0], self._bounds["c1_mak"][1], vs.dmpv.shape), dtype=float
                )[2:-2, 2:-2],
            )
            vs.c2_mak = update(
                vs.c2_mak,
                at[2:-2, 2:-2],
                npx.array(
                    random_uniform(self._bounds["c2_mak"][0], self._bounds["c2_mak"][1], vs.dmpv.shape), dtype=float
                )[2:-2, 2:-2],
            )
            vs.dmpv = update(
                vs.dmpv,
                at[2:-2, 2:-2],
                npx.array(random_uniform(self._bounds["dmpv"][0], self._bounds["dmpv"][1], vs.dmpv.shape), dtype=int)[
                    2:-2, 2:-2
                ],
            )
            vs.lmpv = update(
                vs.lmpv,
                at[2:-2, 2:-2],
                npx.array(random_uniform(self._bounds["lmpv"][0], self._bounds["lmpv"][1], vs.lmpv.shape), dtype=int)[
                    2:-2, 2:-2
                ],
            )
            vs.theta_eff = update(
                vs.theta_eff,
                at[2:-2, 2:-2],
                random_uniform(self._bounds["theta_eff"][0], self._bounds["theta_eff"][1], vs.theta_eff.shape)[
                    2:-2, 2:-2
                ],
            )
            vs.frac_lp = update(
                vs.frac_lp,
                at[2:-2, 2:-2],
                random_uniform(self._bounds["frac_lp"][0], self._bounds["frac_lp"][1], vs.theta_eff.shape)[2:-2, 2:-2],
            )
            ...
            vs.theta_pwp = update(
                vs.theta_pwp,
                at[2:-2, 2:-2],
                random_uniform(self._bounds["theta_pwp"][0], self._bounds["theta_pwp"][1], vs.theta_pwp.shape)[
                    2:-2, 2:-2
                ],
            )
            vs.ks = update(
                vs.ks,
                at[2:-2, 2:-2],
                random_uniform(self._bounds["ks"][0], self._bounds["ks"][1], vs.ks.shape)[2:-2, 2:-2],
            )
