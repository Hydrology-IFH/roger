...
@click.option(
    "-y",
    "--year",
    type=click.Choice(["1997", "1998", "1999", "2000", "2001", "2002", "2003", "2004", "2005", "2006"]),
    default="1999",
)
@click.option(
    "-tms",
...
def main(year, transport_model_structure, sas_solver, tmp_dir):
    ...
    class SVATBROMIDESetup(RogerSetup):
        ...
        _sim_hm_file = f"SVAT_best_for_{transport_model_structure}.nc"
        ...
        def _set_bromide_input(self, state, nn_rain, nn_sol, inf):
            ...
            idx = update(
                idx,
                at[2:-2, 2:-2, :],
                npx.arange(idx.shape[-1])[npx.newaxis, npx.newaxis, :],
            )
            ...
            for i in range(nn_sol):
                input_itt = npx.nanargmin(
                    npx.where(inf_idx[2:-2, 2:-2, :] - sol_idx[i] < 0, npx.nan, inf_idx[2:-2, 2:-2, :] - sol_idx[i]),
                    axis=-1,
                )
                for x in range(input_itt.shape[0]):
                    for y in range(input_itt.shape[1]):
                        ...
                        end_inf = (
                            npx.max(
                                npx.where(
                                    npx.cumsum(inf[x + 2, y + 2, start_inf:]) <= 40,
                                    npx.arange(inf.shape[-1])[start_inf:],
                                    0,
                                )
                            )
                            + 1
                        ...
                        M_IN = update(
                            M_IN,
                            at[x + 2, y + 2, start_inf:end_inf],
                            vs.M_IN[x + 2, y + 2, sol_idx[i]]
                            * (inf[x + 2, y + 2, start_inf:end_inf] / npx.sum(inf[x + 2, y + 2, start_inf:end_inf])),
                        )

            C_IN = update(
                C_IN,
                at[2:-2, 2:-2, :],
                npx.where(inf[2:-2, 2:-2, :] > 0, M_IN[2:-2, 2:-2, :] / inf[2:-2, 2:-2, :], 0),
            )
        ...
        def set_settings(self, state):
            ...
            settings.identifier = f"SVATBROMIDE_{transport_model_structure}_{year}_{sas_solver}"
        ...
        def set_parameters_setup(self, state):
            ...
            vs.S_pwp_rz = update(
                vs.S_pwp_rz,
                at[2:-2, 2:-2],
                self._read_var_from_nc("S_pwp_rz", self._input_dir, self._sim_hm_file)[npx.newaxis, :, 0],
            )
            vs.S_pwp_ss = update(
                vs.S_pwp_ss,
                at[2:-2, 2:-2],
                self._read_var_from_nc("S_pwp_ss", self._input_dir, self._sim_hm_file)[npx.newaxis, :, 0],
            )
            vs.S_sat_rz = update(
                vs.S_sat_rz,
                at[2:-2, 2:-2],
                self._read_var_from_nc("S_sat_rz", self._input_dir, self._sim_hm_file)[npx.newaxis, :, 0],
            )
            vs.S_sat_ss = update(
                vs.S_sat_ss,
                at[2:-2, 2:-2],
                self._read_var_from_nc("S_sat_ss", self._input_dir, self._sim_hm_file)[npx.newaxis, :, 0],
            )
            ...
            if settings.tm_structure == "complete-mixing":
                ...
                vs.sas_params_cpr_rz = update(vs.sas_params_cpr_rz, at[2:-2, 2:-2, 1], 0.2)
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 0], 6)
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], 0.53)
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 0], 6)
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 1], 2.15)
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 0], 6)
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 1], 2.82)
                ...
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 3], 0.44)
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 4], 2.02)
                ...
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 3], 1.15)
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 4], 2.24)
                ...
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 3], 1.31)
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 4], 5.19)
        ...
        def set_initial_conditions_setup(self, state):
            ...
            vs.S_rz = update(
                vs.S_rz,
                at[2:-2, 2:-2, 0],
                self._read_var_from_nc("S_rz", self._input_dir, self._sim_hm_file)[npx.newaxis, :, vs.itt],
            )
            vs.S_ss = update(
                vs.S_ss,
                at[2:-2, 2:-2, 0],
                self._read_var_from_nc("S_ss", self._input_dir, self._sim_hm_file)[npx.newaxis, :, vs.itt],
            )
            vs.S_rz = update(
                vs.S_rz,
                at[2:-2, 2:-2, 1],
        ...
        def set_forcing_setup(self, state):
            ...
            vs.PREC_DIST_DAILY = update(
                vs.PREC_DIST_DAILY,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("prec", self._input_dir, self._sim_hm_file),
            )
            vs.INF_MAT_RZ = update(
                vs.INF_MAT_RZ,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("inf_mat_rz", self._input_dir, self._sim_hm_file),
            )
            vs.INF_PF_RZ = update(
                vs.INF_PF_RZ,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("inf_mp_rz", self._input_dir, self._sim_hm_file)
                + self._read_var_from_nc("inf_sc_rz", self._input_dir, self._sim_hm_file),
            )
            vs.INF_PF_SS = update(
                vs.INF_PF_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_ss", self._input_dir, self._sim_hm_file)
            )
            vs.TRANSP = update(
                vs.TRANSP, at[2:-2, 2:-2, :], self._read_var_from_nc("transp", self._input_dir, self._sim_hm_file)
            )
            vs.EVAP_SOIL = update(
                vs.EVAP_SOIL,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("evap_soil", self._input_dir, self._sim_hm_file),
            )
            vs.CPR_RZ = update(
                vs.CPR_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("cpr_rz", self._input_dir, self._sim_hm_file)
            )
            vs.Q_RZ = update(
                vs.Q_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("q_rz", self._input_dir, self._sim_hm_file)
            )
            vs.Q_SS = update(
                vs.Q_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("q_ss", self._input_dir, self._sim_hm_file)
            )
            vs.S_RZ = update(
                vs.S_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_rz", self._input_dir, self._sim_hm_file)
            )
            vs.S_SS = update(
                vs.S_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_ss", self._input_dir, self._sim_hm_file)
            )
            ...
            TA = update(
                TA,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("ta", self._input_dir, self._sim_hm_file)[npx.newaxis, :, :],
            )

            vs.M_IN = update(
                vs.M_IN, at[2:-2, 2:-2, 1:], self._read_var_from_nc("Br", self._input_dir, "forcing_tracer.nc")
            )
        ...
        @roger_routine
        def set_diagnostics(self, state, base_path=Path(__file__).parent / "output"):
    ...
    idx = pd.date_range(start=idx_start, end=idx_end, freq="D")
    ...
    nc_file = model._base_path / model._sim_hm_file
    with xr.open_dataset(nc_file, engine="h5netcdf") as ds:
        ...
        days_year = date2num(
            ds_year["Time"].values.astype("M8[ms]").astype("O"),
            units=f"days since {int(year) - 1}-12-31",
            calendar="standard",
        )
        ...
        nc_file_year = model._base_path / "input" / str(year) / model._sim_hm_file
