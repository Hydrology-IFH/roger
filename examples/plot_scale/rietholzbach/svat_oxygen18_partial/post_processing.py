...
def main(sas_solver, tmp_dir):
    ...
    tm_structures = ['advection-dispersion-power', 'preferential-power', 'older-preference-power']
    for tm_structure in tm_structures:
        ...
        norm = Normalize(vmin=onp.min(ds_params['k_q_ss'].isel(y=0).values), vmax=onp.max(ds_params['k_q_ss'].isel(y=0).values))
    ...
    tm_structure = 'time-variant advection-dispersion-power'
    tms = tm_structure.replace(" ", "_")
    # load transport simulation
    states_tm_file = base_path / sas_solver / age_max / metric_for_optimization / f"SVATTRANSPORT_{tms}_deterministic.average.nc"
    ds_sim_tm = xr.open_dataset(states_tm_file, engine="h5netcdf")
    ...
    c1_q_ss = onp.unique(ds_params['c1_q_ss'].isel(y=0).values)
    c2_q_ss = onp.unique(ds_params['c2_q_ss'].isel(y=0).values)
    params = onp.array(onp.meshgrid(c1_q_ss, c2_q_ss)).T.reshape(-1, 2)
    c1_q_ss = params[:, 0].tolist()
    c2_q_ss = params[:, 1].tolist()
    cmap = cm.get_cmap('Reds')
    norm = Normalize(vmin=onp.min(ds_params['k_q_ss'].isel(y=0).values), vmax=onp.max(ds_params['k_q_ss'].isel(y=0).values))

    for c1, c2 in zip(c1_q_ss, c2_q_ss):
        rows = onp.where((ds_params['c1_q_ss'].isel(y=0).values == c1) & (ds_params['c2_q_ss'].isel(y=0).values == c2))[0]
        fig, ax = plt.subplots(5, 1, sharey=False, figsize=(6, 6))
        ax.flatten()[0].plot(ds_sim_tm['Time'].values,
                             ds_sim_tm['C_iso_in'].isel(x=0, y=0).values,
                             '-', color='blue')
