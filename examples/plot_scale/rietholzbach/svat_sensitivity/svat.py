...
# simulations with Saltelli's extension of the Sobol' sequence
NSAMPLES = 2**4
BOUNDS = {
    'num_vars': 6,
...
PARAMS = saltelli.sample(BOUNDS, NSAMPLES)
...
class SVATSetup(RogerSetup):
    ...
    def set_settings(self, state):
        ...
        settings.nx, settings.ny, settings.nz = NROWS, 1, 1
    ...
    def set_parameters(self, state):
        ...
        if (vs.itt == 0):
            ...
            vs.dmpv = update(vs.dmpv, at[2:-2, :], npx.repeat(PARAMS[:, 0, npx.newaxis], vs.dmpv.shape[1], axis=-1))
            vs.dmpv = update(vs.dmpv, at[:2, :], 50)  # fill ghosts
            vs.dmpv = update(vs.dmpv, at[-2:, :], 50)
            vs.lmpv = update(vs.lmpv, at[2:-2, :], npx.repeat(PARAMS[:, 1, npx.newaxis], vs.lmpv.shape[1], axis=-1))
            vs.lmpv = update(vs.lmpv, at[:2, :], 400)
            vs.lmpv = update(vs.lmpv, at[-2:, :], 400)
            vs.theta_ac = update(vs.theta_ac, at[2:-2, :], npx.repeat(PARAMS[:, 2, npx.newaxis], vs.theta_ac.shape[1], axis=-1))
            vs.theta_ac = update(vs.theta_ac, at[:2, :], 0.16)
            vs.theta_ac = update(vs.theta_ac, at[-2:, :], 0.16)
            vs.theta_ufc = update(vs.theta_ufc, at[2:-2, :], npx.repeat(PARAMS[:, 3, npx.newaxis], vs.theta_ufc.shape[1], axis=-1))
            vs.theta_ufc = update(vs.theta_ufc, at[:2, :], 0.2)
            vs.theta_ufc = update(vs.theta_ufc, at[-2:, :], 0.2)
            vs.theta_pwp = update(vs.theta_pwp, at[2:-2, :], npx.repeat(PARAMS[:, 4, npx.newaxis], vs.theta_pwp.shape[1], axis=-1))
            vs.theta_pwp = update(vs.theta_pwp, at[:2, :], 0.24)
            vs.theta_pwp = update(vs.theta_pwp, at[-2:, :], 0.24)
            vs.ks = update(vs.ks, at[2:-2, :], npx.repeat(PARAMS[:, 5, npx.newaxis], vs.ks.shape[1], axis=-1))
...
write_forcing(input_path, nrows=NROWS + 4, ncols=5)
model.setup()
model.run()
...
# date_sim = num2date(days - 1, units=f"days since {ds_sim['Time'].attrs['time_origin']}", calendar='standard', only_use_cftime_datetimes=False)
# date_obs = num2date(days - 1, units=f"days since {ds_obs['time'].attrs['time_origin']}", calendar='standard', only_use_cftime_datetimes=False)
...
# vars_obs = ['aet', 'q_ss', 'theta', 'dS_s', 'dS']
