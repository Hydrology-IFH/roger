...
def main(split_size, transport_model_structure, sas_solver, tmp_dir):
    ...
    for diagnostic in diagnostics:
        ...
        if diag_files:
            diag_file = base_path / sas_solver / age_max / f"SVATTRANSPORT_{transport_model_structure}_{sas_solver}.{diagnostic}.nc"
            if not os.path.exists(diag_file):
                ...
                with h5netcdf.File(diag_file, 'a', decode_vlen_strings=False) as f:
                    for i, dfs in enumerate(diag_files):
                        x1 = x1x2[i]
                        x2 = x1x2[i+1]
                        file = base_path / sas_solver / age_max / f"SVATTRANSPORT_{transport_model_structure}_{sas_solver}_{x1}_{x2}.{diagnostic}.nc"
    ...
    path = str(base_path / sas_solver / age_max / f"SVATTRANSPORT_{transport_model_structure}_{sas_solver}.*.nc")
    diag_files = glob.glob(path)
    states_tm_file = base_path / sas_solver / age_max / f"states_{transport_model_structure}_saltelli.nc"
    ...
    if tms == "complete-mixing":
        ...
        df_params_metrics.loc[:, 'b2_transp'] = ds_params["c1_transp"].isel(y=0).values.flatten() + ds_params["c2_transp"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'a_q_rz'] = ds_params["a_q_rz"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'b1_q_rz'] = ds_params["c1_q_rz"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'b2_q_rz'] = ds_params["c1_q_rz"].isel(y=0).values.flatten() + ds_params["c2_q_rz"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'a1_q_ss'] = ds_params["c1_q_ss"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'a2_q_ss'] = ds_params["c1_q_ss"].isel(y=0).values.flatten() + ds_params["c2_q_ss"].isel(y=0).values.flatten()
        df_params_metrics.loc[:, 'b_q_ss'] = ds_params["b_q_ss"].isel(y=0).values.flatten()
    elif tms == "power":
