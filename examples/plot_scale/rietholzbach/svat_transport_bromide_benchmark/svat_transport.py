...
@click.option("-tms", "--transport-model-structure", type=click.Choice(['complete-mixing', 'advection-dispersion', 'time-variant_advection-dispersion', 'power', 'time-variant_power']), default='complete-mixing')
...
def main(transport_model_structure, sas_solver, tmp_dir):
    ...
    class SVATTRANSPORTSetup(RogerSetup):
        ...
        def _set_bromide_input(self, state, nn_rain, nn_sol, prec, ta):
            ...
            for i in range(nn_sol):
                ...
                end_rain = update(end_rain, at[:], start_rain + npx.where(npx.sum(prec[:, :, start_rain:start_rain + nn_end]) <= 0, nn_end + 1, nn_end))
        ...
        def set_parameters_setup(self, state):
            ...
            # elif settings.tm_structure in ["advection-dipsersion", "time-variant advection-dipsersion"]:
            #     vs.sas_params_evap_soil = update(vs.sas_params_evap_soil, at[2:-2, 2:-2, 0], 3)
            #     vs.sas_params_evap_soil = update(vs.sas_params_evap_soil, at[2:-2, 2:-2, 1], 1)
            #     vs.sas_params_evap_soil = update(vs.sas_params_evap_soil, at[2:-2, 2:-2, 2], 100)
            #     vs.sas_params_cpr_rz = update(vs.sas_params_cpr_rz, at[2:-2, 2:-2, 0], 3)
    ...
    for year in years:
        ...
        nc_file = model._base_path / "states_hm.nc"
        with xr.open_dataset(nc_file, engine="h5netcdf") as ds:
            ...
            nc_file_year = model._base_path / "input" / str(year) / "states_hm.nc"
