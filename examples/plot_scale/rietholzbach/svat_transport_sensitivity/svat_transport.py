    ...
    class SVATTRANSPORTSetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            if settings.tm_structure == "complete-mixing":
                ...
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 0], 3)
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], 1)
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 2], self._read_var_from_nc("b_transp", self._base_path, 'params_saltelli.nc', group=self._tm_structure)[x1:x2, :])
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 0], 3)
                ...
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 4], self._read_var_from_nc("c2_transp", self._base_path, 'params_saltelli.nc', group=self._tm_structure)[x1:x2, :])
