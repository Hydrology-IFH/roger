...
def main(job_type):
    ...
    base_path_binac = '/home/fr/fr_fr/fr_rs1092/roger/examples/plot_scale/rietholzbach/svat_transport_monte_carlo_reverse'
    ...
    for tm in transport_models:
        if job_type == 'serial':
            ...
            output_path_ws = base_path_ws / 'rietholzbach' / 'svat_transport_monte_carlo_reverse'
            tms = tm.replace(" ", "_")
            lines = []
            ...
            lines.append(' \n')
            lines.append('python svat_transport.py -b numpy -d cpu -ns 250 -tms %s -td "${TMPDIR}"\n' % (tms))
            ...
            lines.append(' \n')
            lines.append('# adapt command to your available scheduler / MPI implementation\n')
            lines.append(f'mpirun --bind-to core --map-by core -report-bindings python svat_transport.py -b numpy -d cpu -n 100 1 -ns 1000 -tms {tms} -td {output_path_ws.as_posix()}\n')
            file_path = base_path / f'{script_name}_moab.sh'
            file = open(file_path, "w")
            file.writelines(lines)
            file.close()
            ...
            lines.append(f'cd {base_path_binac}\n')
            lines.append(' \n')
            lines.append('python svat_transport.py -b jax -d gpu -ns 1000 -tms %s -td "${TMPDIR}"\n' % (tms))
            lines.append('# Move output from local SSD to global workspace\n')
            lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
            lines.append('mkdir -p %s\n' % (output_path_ws.as_posix()))
            lines.append('mv "${TMPDIR}"/*.nc %s\n' % (output_path_ws.as_posix()))
