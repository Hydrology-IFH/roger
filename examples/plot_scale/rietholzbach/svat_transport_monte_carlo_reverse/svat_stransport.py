...
class SVATTRANSPORTSetup(RogerSetup):
    ...
    def _get_nitt(self):
        nc_file = self._base_path / 'states_hm_monte_carlo.nc'
        with h5netcdf.File(nc_file, "r", decode_vlen_strings=False) as infile:
    ...
    def set_parameters_setup(self, state):
        ...
        if (vs.itt == 0):
            vs.S_PWP_RZ = update(vs.S_PWP_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_rz", 'states_hm_monte_carlo.nc'))
            vs.S_PWP_SS = update(vs.S_PWP_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_ss", 'states_hm_monte_carlo.nc'))
            vs.S_SAT_RZ = update(vs.S_SAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_rz", 'states_hm_monte_carlo.nc'))
            vs.S_SAT_SS = update(vs.S_SAT_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_ss", 'states_hm_monte_carlo.nc'))
        ...
        if settings.tm_structure == "complete-mixing":
            ...
            vs.sas_params_evap_soil = update(vs.sas_params_evap_soil, at[2:-2, 2:-2, 0], 21)
            vs.sas_params_cpr_rz = update(vs.sas_params_cpr_rz, at[2:-2, 2:-2, 0], 21)
            vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_transp", f'sas_parameters_{settings.tm_structure}.nc'))
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_q_rz", f'sas_parameters_{settings.tm_structure}.nc'))
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_q_ss", f'sas_parameters_{settings.tm_structure}.nc'))
    ...
    def set_initial_conditions_setup(self, state):
        ...
        vs.S_RZ = update(vs.S_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_rz", 'states_hm_monte_carlo.nc'))
        vs.S_SS = update(vs.S_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_ss", 'states_hm_monte_carlo.nc'))
    ...
    def set_forcing_setup(self, state):
        ...
        vs.TA = update(vs.TA, at[2:-2, 2:-2, :], self._read_var_from_nc("ta", 'states_hm_monte_carlo.nc'))
        vs.PREC = update(vs.PREC, at[2:-2, 2:-2, :], self._read_var_from_nc("prec", 'states_hm_monte_carlo.nc'))
        vs.INF_MAT_RZ = update(vs.INF_MAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_mat_rz", 'states_hm_monte_carlo.nc'))
        vs.INF_PF_RZ = update(vs.INF_PF_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_mp_rz", 'states_hm_monte_carlo.nc') + self._read_var_from_nc("inf_sc_rz", 'states_hm_monte_carlo.nc'))
        vs.INF_PF_SS = update(vs.INF_PF_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_ss", 'states_hm_monte_carlo.nc'))
        vs.TRANSP = update(vs.TRANSP, at[2:-2, 2:-2, :], self._read_var_from_nc("transp", 'states_hm_monte_carlo.nc'))
        vs.EVAP_SOIL = update(vs.EVAP_SOIL, at[2:-2, 2:-2, :], self._read_var_from_nc("evap_soil", 'states_hm_monte_carlo.nc'))
        vs.Q_RZ = update(vs.Q_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("q_rz", 'states_hm_monte_carlo.nc'))
        vs.Q_SS = update(vs.Q_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("q_ss", 'states_hm_monte_carlo.nc'))
...
for tm_structure in tm_structures:
    ...
    states_tm_file = model._base_path / f"states_tm_{tms}_monte_carlo_reverse.nc"
