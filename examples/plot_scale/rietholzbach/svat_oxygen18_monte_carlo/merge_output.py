...
def main(nsamples, transport_model_structure, split_size, sas_solver, tmp_dir):
    ...
    #     path = str(base_path / "output" / f"SVATTRANSPORT_{transport_model_structure}_{sas_solver}_*.{diagnostic}.nc")
    #     diag_files = glob.glob(path)
    #     if diag_files:
    #         diag_file = base_path / "output" / f"SVATTRANSPORT_{transport_model_structure}_{sas_solver}.{diagnostic}.nc"
    ...
    states_hm1_file = base_path.parent / "svat_monte_carlo" / "output" / f"states_hm{nruns_hm}_bootstrap.nc"
    if os.path.exists(states_hm1_file):
        states_hm_mc_file = base_path / "output" / "states_hm_for_tm_mc.nc"
