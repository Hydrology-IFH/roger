...
#SBATCH --job-name=svat18O_opp_mc
#SBATCH --output=svat18O_opp_mc.out
#SBATCH --error=svat18O_opp_mc_err.out
#SBATCH --export=ALL
...
mpirun --bind-to core --map-by core -report-bindings python svat_oxygen18.py -b jax -d cpu -n 25 1 --float-type float64 -ns 10000 -tms older-preference-power -td "${TMPDIR}"
