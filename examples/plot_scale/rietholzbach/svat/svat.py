...
class SVATSetup(RogerSetup):
    ...
    def set_look_up_tables(self, state):
        ...
        vs.lut_ilu = update(vs.lut_ilu, at[2:-2, 2:-2], lut.ARR_ILU)
        vs.lut_gc = update(vs.lut_gc, at[2:-2, 2:-2], lut.ARR_GC)
        vs.lut_gcm = update(vs.lut_gcm, at[2:-2, 2:-2], lut.ARR_GCM)
        vs.lut_is = update(vs.lut_is, at[2:-2, 2:-2], lut.ARR_IS)
        vs.lut_rdlu = update(vs.lut_rdlu, at[2:-2, 2:-2], lut.ARR_RDLU)
    ...
    def set_parameters_setup(self, state):
        ...
        vs.dmpv = update(vs.dmpv, at[2:-2, 2:-2], 295)
        vs.lmpv = update(vs.lmpv, at[2:-2, 2:-2], 685)
        vs.theta_ac = update(vs.theta_ac, at[2:-2, 2:-2], 0.29294921875)
        vs.theta_ufc = update(vs.theta_ufc, at[2:-2, 2:-2], 0.28638671875000005)
        vs.theta_pwp = update(vs.theta_pwp, at[2:-2, 2:-2],  0.27955078125000005)
        vs.ks = update(vs.ks, at[2:-2, 2:-2], 2.266162109375)
...
def after_timestep_kernel(state):
    ...
    vs.S_fp_rz = update(
        vs.S_fp_rz,
        at[2:-2, 2:-2], npx.where((vs.S_fp_rz > -1e-6) & (vs.S_fp_rz < 0), 0, vs.S_fp_rz),
    )
    vs.S_lp_rz = update(
        vs.S_lp_rz,
        at[2:-2, 2:-2], npx.where((vs.S_lp_rz > -1e-6) & (vs.S_lp_rz < 0), 0, vs.S_lp_rz),
    )
    vs.S_fp_ss = update(
        vs.S_fp_ss,
        at[2:-2, 2:-2], npx.where((vs.S_fp_ss > -1e-6) & (vs.S_fp_ss < 0), 0, vs.S_fp_ss),
    )
    vs.S_lp_ss = update(
        vs.S_lp_ss,
        at[2:-2, 2:-2], npx.where((vs.S_lp_ss > -1e-6) & (vs.S_lp_ss < 0), 0, vs.S_lp_ss),
    )
