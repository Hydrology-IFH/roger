...
if not os.path.exists(base_path_results):
    os.mkdir(base_path_results)
base_path_results = base_path / "results" / "RK4" / age_max
if not os.path.exists(base_path_results):
    os.mkdir(base_path_results)
...
if not os.path.exists(base_path_figs):
    os.mkdir(base_path_figs)
base_path_figs = base_path / "figures" / "RK4" / age_max
if not os.path.exists(base_path_figs):
    os.mkdir(base_path_figs)
...
for tm in transport_models:
    path = str(base_path / "RK4" / age_max / f"SVATTRANSPORT_{tm}_RK4.*.nc")
    diag_files = glob.glob(path)
    tm1 = tm.replace(" ", "_")
    states_tm_file = base_path / "RK4" / age_max / f"states_{tm1}_RK4.nc"
