...
def main(tmp_dir):
    ...
    class SVATSetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            # soil depth (mm)
            vs.z_soil = update(vs.z_soil, at[2:-2, 2:-2], self._read_var_from_nc("z_soil", self._base_path, 'parameters.nc'))
            ...
            # air capacity (-)
            vs.theta_ac = update(vs.theta_ac, at[2:-2, 2:-2], self._read_var_from_nc("theta_ac", self._base_path, 'parameters.nc'))
            # usable field capacity (-)
            vs.theta_ufc = update(vs.theta_ufc, at[2:-2, 2:-2], self._read_var_from_nc("theta_ufc", self._base_path, 'parameters.nc'))
            # permanent wilting point (-)
            vs.theta_pwp = update(vs.theta_pwp, at[2:-2, 2:-2], self._read_var_from_nc("theta_pwp", self._base_path, 'parameters.nc'))
        ...
        def set_diagnostics(self, state, base_path=tmp_dir):
            ...
            diagnostics["rate"].output_variables = ["prec", "aet", "transp", "evap_soil", "inf_mat_rz", "inf_mp_rz", "inf_sc_rz", "inf_ss", "q_rz", "q_ss", "cpr_rz", "q_snow"]
