...
def main(tmp_dir):
    ...
    class SVATSetup(RogerSetup):
        ...
        def set_look_up_tables(self, state):
            ...
            vs.lut_gc = update(vs.lut_gc, at[-1, 0], 60)
        ...
        def set_parameters_setup(self, state):
            ...
            vs.c_root = update(
                vs.c_root,
                at[2:-2, 2:-2],
                self._read_var_from_csv("c_root", self._base_path, "parameters.csv").reshape(settings.nx, settings.ny),
            )
            vs.c_int = update(
                vs.c_int,
                at[2:-2, 2:-2],
                self._read_var_from_csv("c_int", self._base_path, "parameters.csv").reshape(settings.nx, settings.ny),
            )
