...
if not os.path.exists(states_hm_file):
    for meteo_station in meteo_stations:
        path = str(base_path / f"SVAT_{meteo_station}.*.nc")
...
for i, meteo_station in enumerate(meteo_stations):
    ...
    date = num2date(hours, units=f"hours since {ds_input['Time'].attrs['time_origin']}", calendar='standard', only_use_cftime_datetimes=False)
    ...
    axs[0].plot(days,
                ds_input_avg_daily.TA.values.flatten(),
                '-', color='orange', linewidth=1)
    axs[0].set_ylabel(r'TA [°C]')
    axs[0].set_xlim(days[0], days[-1])
    axs[1].bar(days, ds_input_sum_daily.PREC.values.flatten(), width=.1, color='blue', align='edge', edgecolor='blue')
    axs[1].set_ylabel('PRECIP\n[mm/day]')
    axs[1].set_xlim(days[0], days[-1])
    axs[2].bar(days, ds_input_sum_daily.PET.values.flatten(), width=.1, color='green', align='edge', edgecolor='green')
...
for i, meteo_station in enumerate(meteo_stations):
    ...
    vars_sim_trace = ['S_s', 'theta', 'int_prec', 'q_snow', 
                      'inf_in', 'inf_mat', 'inf_mp', 'inf_sc',
                      'q_hof', 'q_sof', 'q_ss','aet', 'evap_sur',
                      'evap_soil', 'transp']
    nx = ds_sim.dims['x']
    days_sim = ds_sim.Time.values / onp.timedelta64(24 * 60 * 60, "s")
    for j, var_sim in enumerate(vars_sim_trace):
        ...
        ax.fill_between(days_sim[1:], min_vals[1:], max_vals[1:], edgecolor=color, facecolor=color, alpha=.33, label='Min-Max interval')
...
ax = sns.catplot(x="variable", y="value", hue="meteo_station",
                data=df_sim_sum, kind="box", height=3, aspect=2, palette="RdPu", whis=[0, 100])
