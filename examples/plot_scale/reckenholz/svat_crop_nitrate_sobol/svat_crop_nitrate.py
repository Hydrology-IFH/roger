...
@click.option("-td", "--tmp-dir", type=str, default=Path(__file__).parent.parent / "output" / "svat_crop_nitrate_sobol")
@roger_base_cli
def main(lys_experiment, tmp_dir):
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        def set_settings(self, state):
            ...
            settings.identifier = f"SVATCROPNITRATE_advection-dispersion-power_{lys_experiment}"
            settings.sas_solver = "deterministic"
            settings.sas_solver_substeps = 6

            settings.nx, settings.ny = self._get_nsamples(self._base_path, "parameters.nc"), 1
        ...
        def set_parameters_setup(self, state):
            ...
            vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_transp", self._base_path, "parameters.nc"))
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 0], 6)
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_q", self._base_path, "parameters.nc"))
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 0], 6)
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_q", self._base_path, "parameters.nc"))
