...
@click.option("-ns", "--nsamples", type=int, default=2**1)
@click.command("main")
def main(nsamples):
    ...
    for transport_model_structure in ['advection-dispersion-power', 'time-variant_advection-dispersion-power']:
        # load parameter boundaries
        file_param_bounds = base_path / "param_bounds.yml"
        with open(file_param_bounds, "r") as file:
            dict_bounds = yaml.safe_load(file)[transport_model_structure]
        ...
        if transport_model_structure == 'advection-dispersion-power':
            ...
            param_names = ["alpha_transp", "alpha_q", "c1_transp", "c2_transp", "c1_q_rz", "c2_q_rz", "c1_q_ss", "c2_q_ss", "km_denit", "km_nit", "kmin", "kfix", "kngl", "dmax_denit", "dmax_nit", "phi_soil_temp", "clay", "soil_fertility", "z_soil"]
            df_params = df_params.loc[:, param_names]
        ...
        params_file = base_path / f"parameters_for_{transport_model_structure}.nc"
