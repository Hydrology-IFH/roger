...
for lys_experiment in lys_experiments:
    ...
    states_hm_file = base_path / "output" / "svat_crop_nitrate_sobol" / f"SVATCROP_{lys_experiment}.nc"
    ...
    for tm_structure in tm_structures:
        ...
        states_tm_file = base_path / "output" / "svat_crop_nitrate_sobol" / f"SVATCROPNITRATE_{tms}_{lys_experiment}.nc"
        ...
        for nrow in range(nx):
            ...
            if cond.all():
                ...
                key = f'{var_sim}_annual_sum'
                df_params_metrics.loc[nrow, key] = onp.mean(sim_vals)

                var_sim = 'M_q_ss'
                sim_vals = ds_sim_tm[var_sim].isel(x=nrow, y=0).values[1:]
                key = f'{var_sim}_daily_avg'
                df_params_metrics.loc[nrow, key] = onp.nanmean(sim_vals)
        ...
        # write to .txt
        file = base_path / "output" / "svat_crop_nitrate_sobol" / f"params_metrics_{lys_experiment}_{tm_structure}.txt"
        df_params_metrics.to_csv(file, header=True, index=False, sep="\t")

        file = base_path / "output" / "svat_crop_nitrate_sobol" / f"params_metrics_{lys_experiment}_{tm_structure}.csv"
        df_params_metrics.to_csv(file, header=True, index=False, sep=";")
