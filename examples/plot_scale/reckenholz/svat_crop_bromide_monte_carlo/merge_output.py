...
for lys_experiment in lys_experiments:
    for tm_structure in tm_structures:
        path = str(
            base_path.parent
            / "output"
            / "svat_crop_bromide_monte_carlo"
            / f"SVATCROPBR_{tm_structure}_{lys_experiment}.*.nc"
        )
        output_tm_file = (
            base_path.parent
            / "output"
        ...
        if not os.path.exists(output_tm_file):
            ...
            with h5netcdf.File(output_tm_file, "w", decode_vlen_strings=False) as f:
                f.attrs.update(
                    ...
                    title=f"RoGeR Monte Carlo simulations of bromide transport at Reckenholz lysimeter ({lys_experiment})",
