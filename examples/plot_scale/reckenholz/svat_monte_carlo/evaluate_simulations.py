    ...
    for lys_experiment in lys_experiments:
        ...
        for var_sim, var_obs in zip(vars_sim, vars_obs):
            ...
            for nrow in range(len(df_params_metrics.index)):
                ...
                df_params_metrics.loc[nrow, "N"] = N_obs
                if N_obs > 30:
                    if var_sim in ["theta"]:
        ...
        if "r_theta" in df_params_metrics.columns:
            df_params_metrics.loc[:, "E_multi"] = (
                1 / 2 * df_params_metrics.loc[:, "r_theta"] + 1 / 2 * df_params_metrics.loc[:, "KGE_q_ss"]
            )
        ...
        # write states of best model run
        states_hm_mc_file = base_path_output / f"SVAT_{lys_experiment}.nc"
        states_hm_file = base_path_output / f"SVAT_{lys_experiment}_best_100_simulations.nc"
        with h5netcdf.File(states_hm_file, "a", decode_vlen_strings=False) as f:
            if lys_experiment not in list(f.keys()):
                f.create_group(lys_experiment)
            f.attrs.update(
