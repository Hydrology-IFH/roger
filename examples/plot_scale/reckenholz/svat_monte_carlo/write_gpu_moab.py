...
for lys in lysimeters:
    ...
    lines.append('python svat_crop.py -b jax -d gpu -lys %s -td "${TMPDIR}"\n' % (lys))
    ...
    file_path = base_path / f'{script_name}_gpu_moab.sh'
    file = open(file_path, "w")
    file.writelines(lines)
    file.close()
    subprocess.Popen(f"chmod +x {script_name}_gpu_moab.sh", shell=True)
