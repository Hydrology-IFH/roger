...
class SVATCROPTRANSPORTSetup(RogerSetup):
    ...
    def _get_nx(self, path_dir, file, group=None):
        ...
        if group:
            with h5netcdf.File(nc_file, "r", decode_vlen_strings=False) as infile:
                var_obj = infile.groups[group].variables['x']
    ...
    def set_parameters_setup(self, state):
        ...
        vs.alpha_transp = update(vs.alpha_transp, at[2:-2, 2:-2], self._read_var_from_nc("alpha_transp", self._base_path, 'tm_bromide_params.nc', group=self._lys, subgroup=settings.tm_structure))
        vs.alpha_q = update(vs.alpha_q, at[2:-2, 2:-2], self._read_var_from_nc("alpha_q", self._base_path, 'tm_bromide_params.nc', group=self._lys, subgroup=settings.tm_structure))
        ...
        if settings.tm_structure == "complete-mixing":
            ...
            vs.sas_params_evap_soil = update(vs.sas_params_evap_soil, at[2:-2, 2:-2, 0], 21)
            vs.sas_params_cpr_rz = update(vs.sas_params_cpr_rz, at[2:-2, 2:-2, 0], 21)
            vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_transp", self._base_path, 'tm_bromide_params.nc', group=self._lys, subgroup=settings.tm_structure))
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_q_rz", self._base_path, 'tm_bromide_params.nc', group=self._lys, subgroup=settings.tm_structure))
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, :], self._read_var_from_nc("sas_params_q_ss", self._base_path, 'tm_bromide_params.nc', group=self._lys, subgroup=settings.tm_structure))
            vs.sas_params_re_rg = update(vs.sas_params_re_rg, at[2:-2, 2:-2, 0], 21)
            vs.sas_params_re_rl = update(vs.sas_params_re_rl, at[2:-2, 2:-2, 0], 22)
