...
def main(job_type, sas_solver):
    ...
    base_path_binac = '/home/fr/fr_fr/fr_rs1092/roger/examples/plot_scale/reckenholz/svat_nitrate_monte_carlo'
    ...
    for lys in lysimeters:
        for tm in transport_models:
            if job_type == 'single-node':
                ...
                tms = tm.replace(" ", "_")
                script_name = f'svat_{tracer}_{tm1}_mc_{lys}'
                output_path_ws = base_path_ws / 'reckenholz' / 'svat_nitrate_monte_carlo'
                lines = []
                ...
                file_nc = "SVAT_%s_bootstrap.nc" % (lys)
                lines.append(
                ...
                lines.append('mpirun --bind-to core --map-by core -report-bindings python svat_nitrate.py -b jax -d cpu -n 25 1 -lys %s -tms %s -td "${TMPDIR}"\n' % (lys, tms))
                ...
                lines.append(" \n")
                lines.append('# adapt command to your available scheduler / MPI implementation\n')
                lines.append('mpirun --bind-to core --map-by core -report-bindings python svat_nitrate.py -b jax -d cpu -n 40 1 -lys %s -tms %s -td "${TMPDIR}"\n' % (lys, tms))
                file_path = base_path / f'{script_name}_moab.sh'
                file = open(file_path, "w")
                file.writelines(lines)
                file.close()
