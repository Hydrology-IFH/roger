...
lys_experiments = ["lys3", "lys8"]
tm_structures = ["complete-mixing"]

for lys_experiment in lys_experiments:
    # load hydrologic simulation
    states_hm_file = base_path / "output" / "svat_monte_carlo" / f"SVAT_{lys_experiment}_bootstrap.nc"
    ...
    for tm_structure in tm_structures:
        ...
        states_tm_file = base_path / "output" / "svat_nitrate_monte_carlo" / f"SVATNITRATE_{tms}_{lys_experiment}.nc"
        ...
        # write to .txt
        file = base_path / "output" / "svat_nitrate_monte_carlo" / f"params_metrics_{lys_experiment}_{tm_structure}.txt"
        df_params_metrics.to_csv(file, header=True, index=False, sep="\t")

        file = base_path / "output" / "svat_nitrate_monte_carlo" / f"params_metrics_{lys_experiment}_{tm_structure}.csv"
        df_params_metrics.to_csv(file, header=True, index=False, sep=";")
