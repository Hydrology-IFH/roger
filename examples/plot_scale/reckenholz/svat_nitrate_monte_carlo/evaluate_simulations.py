...
for lys_experiment in lys_experiments:
    ...
    states_hm_file = base_path / "svat_monte_carlo" / f"SVATCROP_{lys_experiment}_bootstrap.nc"
    ...
    for tm_structure in tm_structures:
        ...
        states_tm_file = base_path / "svat_nitrate_monte_carlo" / f"SVATNITRATE_{tms}_{lys_experiment}.nc"
        ...
        for nrow in range(nx):
            ...
            sim_vals = df_sim_annual.loc[:, 'sim'].values
            obs_vals = onp.sum(ds_obs[var_obs].isel(x=0, y=0).values, axis=0)
            var_sim = 'N_uptake'
            key_kge_alpha = f'KGE_alpha_{var_sim}'
            df_params_metrics.loc[nrow, key_kge_alpha] = eval_utils.calc_kge_alpha(obs_vals, sim_vals)
            key_kge_beta = f'KGE_beta_{var_sim}'
        ...
        # write to .txt
        file = base_path / "svat_nitrate_monte_carlo" / f"params_metrics_{lys_experiment}_{tm_structure}.txt"
        df_params_metrics.to_bsv(file, header=True, index=False, sep="\t")