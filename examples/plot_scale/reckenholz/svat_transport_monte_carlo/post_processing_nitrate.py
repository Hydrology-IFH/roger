    ...
    for tm_structure in tm_structures:
        ...
        with h5netcdf.File(states_tm_file, 'a', decode_vlen_strings=False) as f:
            ...
            f.attrs.update(
                ...
                title='RoGeR nitrate transport model Monte Carlo simulations at Reckenholz Lysimeter site',
...
for lys_experiment in lys_experiments:
    ...
    for tm_structure in tm_structures:
        ...
        NO3_perc_mass_cs = onp.zeros((1, 1, len(idx)))
        ...
        for nrow in range(nx):
            ...
            NO3_perc_cs[nrow, ncol, :] = df_perc_NO3_sim.loc[:, 'NO3_conc'].values
            # mass of simulated composite samples
            NO3_perc_mass_cs[nrow, ncol, :] = df_perc_NO3_sim.loc[:, 'NO3_mass_sum'].values

            # calculate metrics
            vars_sim = ['NO3_perc_cs', 'NO3_perc_mass_cs']
            vars_obs = ['NO3_PERC', 'NO3_PERC_MASS']
            for var_sim, var_obs in zip(vars_sim, vars_obs):
                # join observations on simulations
                obs_vals = ds_obs[var_obs].isel(x=0, y=0).values
                if var_sim == 'NO3_perc_cs':
                    sim_vals = NO3_perc_cs[nrow, ncol, :]
                elif var_sim == 'NO3_perc_mass_cs':
                    sim_vals = NO3_perc_mass_cs[nrow, ncol, :]
        ...
        if tm_structure in ['preferential', 'advection-dispersion',
            ...
            df_eff = df_params_eff.loc[:, ['KGE_NO3_perc_cs']]
            if tm_structure == "preferential":
        ...
        idx_best = df_params_eff['KGE_NO3_perc_cs'].idxmax()
        ...
        with h5netcdf.File(states_tm_file, 'r+', decode_vlen_strings=False) as f:
            try:
                v = f.groups[tm_structure].create_variable('NO3_perc_cs', ('x', 'y', 'Time'), float)
            except ValueError:
                v = f.groups[tm_structure].get('NO3_perc_cs')
            v[:, :, :] = NO3_perc_cs
            v.attrs.update(long_name="composite sample of nitrate concentration in percolation",
                           units="mg/l")
            try:
                v = f.groups[tm_structure].create_variable('NO3_perc_mass_cs', ('x', 'y', 'Time'), float)
            except ValueError:
                v = f.groups[tm_structure].get('NO3_perc_mass_cs')
            v[:, :, :] = NO3_perc_mass_cs
            v.attrs.update(long_name="composite sample of nitrate mass in percolation",
                           units="mg")
...
for i, tm_structure in enumerate(tm_structures):
    ...
    sim_vals = ds_sim_tm['NO3_perc_cs'].isel(x=idx_best, y=0).values
    df_obs = pd.DataFrame(index=date_obs, columns=['obs'])
    df_obs.loc[:, 'obs'] = obs_vals
