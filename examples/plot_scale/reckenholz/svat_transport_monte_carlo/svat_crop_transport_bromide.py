...
def main(nsamples, lys_experiment, transport_model_structure, sas_solver, crop_partitioning):
    ...
    class SVATCROPTRANSPORTSetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            if settings.enable_crop_partitioning:
                ...
                vs.alpha_transp = update(vs.alpha_transp, at[2:-2, 2:-2], random_uniform(self._bounds['alpha_transp'][0], self._bounds['alpha_transp'][1], tuple((vs.alpha_transp.shape[0], vs.alpha_transp.shape[1])))[2:-2, 2:-2]
            vs.alpha_q = update(vs.alpha_q, at[2:-2, 2:-2], random_uniform(self._bounds['alpha_q'][0], self._bounds['alpha_q'][1], tuple((vs.alpha_q.shape[0], vs.alpha_q.shape[1])))[2:-2, 2:-2]
    ...
    if sas_solver in ['RK4', 'Euler']:
        identifier = f'SVATCROPTRANSPORT_{lys_experiment}_{tms}_{sas_solver}_bromide'
    else:
        identifier = f'SVATCROPTRANSPORT_{lys_experiment}_{tms}_bromide'
