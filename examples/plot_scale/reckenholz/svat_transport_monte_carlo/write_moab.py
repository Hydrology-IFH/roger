...
def main(job_type, sas_solver):
    ...
    base_path_binac = '/home/fr/fr_fr/fr_rs1092/roger/examples/plot_scale/reckenholz/svat_transport_monte_carlo'
    ...
    for lys in lysimeters:
        for tm in transport_models:
            if job_type == 'single-node':
                ...
                output_path_ws = base_path_ws / 'reckenholz' / 'svat_transport_monte_carlo'
                lines = []
    ...
    tracer = 'nitrate2'
    lysimeters = ['lys2', 'lys3', 'lys4', 'lys8', 'lys9']
    transport_models = ['complete-mixing']
    for lys in lysimeters:
        for tm in transport_models:
            if job_type == 'single-node':
                tm1 = transport_models_abrev[tm]
                tms = tm.replace(" ", "_")
                script_name = f'{tracer}_{lys}_svat_{tm1}_mc'
                output_path_ws = base_path_ws / 'reckenholz' / 'svat_transport_monte_carlo'
                lines = []
                lines.append('#!/bin/bash\n')
                lines.append('#PBS -l nodes=1:ppn=20\n')
                ...
                lines.append(' \n')
                lines.append('# adapt command to your available scheduler / MPI implementation\n')
                lines.append('mpirun --bind-to core --map-by core -report-bindings python svat_transport_%s.py --log-all-processes -b numpy -d cpu -ns 2000 -n 20 1 -lys %s -tms %s -td "${TMPDIR}" -ss %s\n' % (tracer, lys, tms, sas_solver))
                lines.append('# Write output to temporary SSD of computing node\n')
                lines.append('echo "Write output to $TMPDIR"\n')
                lines.append('# Move output from temporary SSD to workspace\n')
                lines.append(f'echo "Move output to {output_path_ws.as_posix()}"\n')
                ...
                lines.append(' \n')
                lines.append('# adapt command to your available scheduler / MPI implementation\n')
                lines.append('mpirun --bind-to core --map-by core -report-bindings python svat_transport_%s.py --log-all-processes -b numpy -d cpu -ns 4000 -n 40 1 -lys %s -tms %s -td %s -ss %s\n' % (tracer, lys, tms, output_path_ws.as_posix(), sas_solver))
                file_path = base_path / f'{script_name}_moab.sh'
                file = open(file_path, "w")
                file.writelines(lines)
                file.close()
