...
def main():
    ...
    for lys_experiment in lys_experiments:
        path = str(base_path.parent / "output" / "svat_crop_nitrate_sobol" / f"SVATCROP_{lys_experiment}.*.nc")
        output_hm_file = base_path.parent / "output" / "svat_crop_nitrate_sobol" / f"SVATCROP_{lys_experiment}.nc"
        if not os.path.exists(output_hm_file):
            diag_files = glob.glob(path)
            with h5netcdf.File(output_hm_file, 'w', decode_vlen_strings=False) as f:
                f.attrs.update(
                    date_created=datetime.datetime.today().isoformat(),
                    title=f'RoGeR Sobol simulations at Reckenholz lysimeter {lys_experiment}',
