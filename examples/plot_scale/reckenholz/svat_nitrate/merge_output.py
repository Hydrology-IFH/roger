...
def main():
    ...
    for lys_experiment in lys_experiments:
        for tm_structure in tm_structures:
            path = str(base_path.parent / "output" / "svat_nitrate" / f"SVATN_{tm_structure}_{lys_experiment}.*.nc")
            output_hm_file = base_path.parent / "output" / "svat_nitrate" / f"SVATN_{tm_structure}_{lys_experiment}.nc"
            if not os.path.exists(output_hm_file):
                diag_files = glob.glob(path)
                with h5netcdf.File(output_hm_file, "w", decode_vlen_strings=False) as f:
                    f.attrs.update(
                        date_created=datetime.datetime.today().isoformat(),
                        title=f"RoGeR nitrate transport simulations at Reckenholz lysimeter {lys_experiment}",
                        institution="University of Freiburg, Chair of Hydrology",
                        references="",
                        comment="First timestep (t=0) contains initial values. Simulations start are written from second timestep (t=1) to last timestep (t=N).",
                        model_structure="SVAT nitrate transport model with free drainage",
