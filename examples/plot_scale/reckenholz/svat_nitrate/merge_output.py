        ...
        for tm_structure in tm_structures:
                ...
                if diag_files:
                    with h5netcdf.File(output_tm_file, "w", decode_vlen_strings=False) as f:
                        tm_structure1 = tm_structure.replace("_", " ")
                        f.attrs.update(
                            date_created=datetime.datetime.today().isoformat(),
                            title=f"RoGeR simulations of water-bound soil nitrate transport at Reckenholz lysimeter site, Switzerland ({lys_experiment})",
                            institution="University of Freiburg, Chair of Hydrology",
                            references="",
                            comment="First timestep (t=0) contains initial values. Simulations start are written from second timestep (t=1) to last timestep (t=N).",
                            model_structure=f"SVAT model with free drainage and with SAS parameters representing ({tm_structure1})",
