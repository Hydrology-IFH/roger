...
def main(lys_experiment, transport_model_structure, tmp_dir):
    ...
    class SVATNITRATESetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            vs.S_PWP_RZ = update(vs.S_PWP_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_rz", self._input_dir1, f'SVAT_{lys_experiment}.nc'))
            vs.S_SAT_RZ = update(vs.S_SAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_rz", self._input_dir1, f'SVAT_{lys_experiment}.nc'))
            vs.S_PWP_SS = update(vs.S_PWP_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_ss", self._input_dir1, f'SVAT_{lys_experiment}.nc'))
            vs.S_SAT_SS = update(vs.S_SAT_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_ss", self._input_dir1, f'SVAT_{lys_experiment}.nc'))
            ...
            vs.km_denit_rz = update(vs.km_denit_rz, at[2:-2, 2:-2], self._read_var_from_nc("km_denit", self._base_path, "parameters.nc") * 2)
            vs.km_denit_ss = update(vs.km_denit_ss, at[2:-2, 2:-2], self._read_var_from_nc("km_denit", self._base_path, "parameters.nc") * 2)
            # nitrification parameters
            vs.dmax_denit_rz = update(vs.dmax_denit_rz, at[2:-2, 2:-2], self._read_var_from_nc("dmax_denit", self._base_path, "parameters.nc") * 2)
            vs.dmax_denit_ss = update(vs.dmax_denit_ss, at[2:-2, 2:-2], self._read_var_from_nc("dmax_denit", self._base_path, "parameters.nc") * 2)
        ...
        def set_forcing(self, state):
            ...
            vs.nup = update(
                vs.nup,
                at[2:-2, 2:-2],
                npx.where(1 * settings.dx * settings.dy * 100, vs.nup[2:-2, 2:-2]),
            )
            vs.nup = update(
                vs.nup,
                at[2:-2, 2:-2],
