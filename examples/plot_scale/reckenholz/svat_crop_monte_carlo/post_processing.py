...
def main(tmp_dir):
    ...
    for lys_experiment in lys_experiments:
        ...
        if not os.path.exists(base_path_output):
            os.mkdir(base_path_output)
        # directory of figures
        base_path_figs = base_path / "figures" / "svat_crop_monte_carlo"
        if not os.path.exists(base_path_figs):
            os.mkdir(base_path_figs)

        # load simulation
        states_hm_mc_file = base_path / f"SVATCROP_{lys_experiment}.nc"
        ...
        df_params_eff = pd.read_csv(csv_file, sep=";", skiprows=1)
        # calculate metrics
        ...
        for var_sim, var_obs in zip(vars_sim, vars_obs):
            ...
            for nrow in range(len(df_params_eff.index)):
                ...
                if N_obs > 30:
                    if var_sim in ["theta"]:
                        ...
                        if cond0.any():
                            ...
                            df_params_eff.loc[nrow, key_mae0] = eval_utils.calc_mae(obs0_vals, sim0_vals)
                            ...
                            df_params_eff.loc[nrow, key_de0] = (N_no0 / N_obs) * df_params_eff.loc[nrow, key_de] + (
                                N_obs0 / N_obs
                            ) * ioa0
                        else:
                            # share of observations with zero values
        ...
        if "r_S" in df_params_eff.columns:
            df_params_eff.loc[:, "E_multi"] = (
                1 / 2 * df_params_eff.loc[:, "r_S"] + 1 / 2 * df_params_eff.loc[:, "KGE_q_ss"]
            )
        ...
        if "E_multi" in df_params_eff.columns:
            ...
            df_params = df_params_eff.loc[
                :, ["dmpv", "lmpv", "theta_ac", "theta_ufc", "theta_pwp", "ks", "c_canopy_growth", "c_root_growth"]
            ]
        ...
        states_hm_mc_file = base_path / f"SVATCROP_{lys_experiment}.nc"
        states_hm_file = base_path / f"SVATCROP_{lys_experiment}_best_simulation.nc"
        with h5netcdf.File(states_hm_file, "a", decode_vlen_strings=False) as f:
            ...
            with h5netcdf.File(states_hm_mc_file, "r", decode_vlen_strings=False) as df:
                ...
                if not f.groups[lys_experiment].dimensions:
                    ...
                    v = f.groups[lys_experiment].create_variable(
                ...
                for var_sim in list(df.variables.keys()):
                    ...
                    if (
                        var_sim not in list(f.groups[lys_experiment].dimensions.keys())
                        and ("x", "y", "Time") == var_obj.dimensions
                    ):
                        v = f.groups[lys_experiment].create_variable(
