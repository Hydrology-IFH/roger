...
def main(tmp_dir):
    ...
    base_path_figs = base_path.parent / "figures"
    ...
    for lys_experiment in lys_experiments:
        # directory of results
        base_path_output = Path("/Volumes/LaCie/roger/examples/plot_scale/reckenholz") / "output" / "svat_crop_monte_carlo"
        if not os.path.exists(base_path_output):
            os.mkdir(base_path_output)
        # directory of figures
        ...
        for var_sim, var_obs in zip(vars_sim, vars_obs):
            ...
            df_cond = pd.DataFrame(index=date_obs)
            df_cond.loc[:, "prec"] = ds_obs["PREC"].isel(x=0, y=0).values
            df_cond.loc[:, "pet"] = ds_obs["PET"].isel(x=0, y=0).values
            df_cond.loc[:, "perc"] = ds_obs["PERC"].isel(x=0, y=0).values
            df_cond.loc[:, "weight"] = ds_obs["WEIGHT"].isel(x=0, y=0).values
            df_cond.loc[df_cond.index[1]:, "dS"] = df_cond.loc[:, "weight"].values[1:] - df_cond.loc[:, "weight"].values[:-1]
            df_cond.loc[:, "perc_pet_ratio"] = df_cond.loc[:, "perc"] / (df_cond.loc[:, "perc"] + df_cond.loc[:, "pet"])
            df_cond = pd.DataFrame(index=date_sim).join(df_cond)
            # condition for plausible lysimeter seepage
            cond_perc = (df_cond.loc[:, "perc_pet_ratio"] < 0.5) & ((df_cond.loc[:, "dS"] > df_cond.loc[:, "prec"]) | (df_cond.loc[:, "dS"] < df_cond.loc[:, "pet"] + df_cond.loc[:, "perc"]))
            # condition for plausible lysimeter weight
            cond_S = (df_cond.loc[:, "dS"] > df_cond.loc[:, "prec"]) | (df_cond.loc[:, "dS"] < df_cond.loc[:, "pet"] + df_cond.loc[:, "perc"])
        ...
        if "r_S" in df_params_metrics.columns:
            df_params_metrics.loc[:, "E_multi"] = (
                1 / 2 * df_params_metrics.loc[:, "r_S"] + 1 / 2 * df_params_metrics.loc[:, "KGE_q_ss"]
            )
