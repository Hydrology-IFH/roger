...
def main(lys_experiment, transport_model_structure, tmp_dir):
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        _input_dir1 = Path("/Volumes/LaCie/roger/examples/plot_scale/reckenholz") / "output" / "svat_crop_monte_carlo"
        ...
        def set_settings(self, state):
            ...
            settings.nx, settings.ny = self._get_nsamples(self._input_dir1, f"SVATCROP_{lys_experiment}_best_simulation.nc"), 1
            settings.nitt = self._get_nitt(
                self._input_dir1, f"SVATCROP_{lys_experiment}_best_simulation.nc"
            )
            settings.nitt_forc = settings.nitt
            settings.ages = 1000
            settings.nages = settings.ages + 1
            settings.runlen_warmup = 2 * 365 * 24 * 60 * 60
            settings.runlen = self._get_runlen(self._input_dir1, f"SVATCROP_{lys_experiment}_best_simulation.nc") - 24 * 60 * 60
        ...
        def set_parameters_setup(self, state):
            ...
            vs.S_PWP_RZ = update(vs.S_PWP_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_SAT_RZ = update(vs.S_SAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_PWP_SS = update(vs.S_PWP_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_pwp_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_SAT_SS = update(vs.S_SAT_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_sat_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))

            vs.S_pwp_rz = update(vs.S_pwp_rz, at[2:-2, 2:-2], self._read_var_from_nc("S_pwp_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, 0])
            vs.S_pwp_ss = update(vs.S_pwp_ss, at[2:-2, 2:-2], self._read_var_from_nc("S_pwp_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, 0])
            vs.S_sat_rz = update(vs.S_sat_rz, at[2:-2, 2:-2], self._read_var_from_nc("S_sat_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, 0])
            vs.S_sat_ss = update(vs.S_sat_ss, at[2:-2, 2:-2], self._read_var_from_nc("S_sat_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, 0])
            ...
            vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_transp_winter-barley", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 0], 6)
            vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_q_winter-barley", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 0], 6)
            vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 1], self._read_var_from_nc("k_q_winter-barley", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            ...
            vs.YEAR = update(
                ...
                self._read_var_from_nc(
                    "year", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'
                ),
            )
            vs.DOY = update(
                vs.DOY,
                at[:],
                self._read_var_from_nc(
                    "doy", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'
                ),
            )
            vs.LU_ID = update(
                vs.LU_ID,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "lu_id", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'
                ),
            )
            vs.Z_ROOT = update(
                vs.Z_ROOT,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "z_root", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'
                ),
            )
        ...
        def set_parameters(self, state):
            ...
            if vs.itt < settings.nitt - 31:
                if (vs.LU_ID[2:-2, 2:-2, vs.itt+30] != vs.LU_ID[2:-2, 2:-2, vs.itt+31]).any():
                    ...
                    # partition coefficients
                    print(f"alpha_transp_{crop}", vs.alpha_transp[2,2])
                    vs.alpha_transp = update(vs.alpha_transp, at[2:-2, 2:-2], self._read_var_from_nc(f"alpha_transp_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    print(f"alpha_transp_{crop}", vs.alpha_transp[2,2])
                    vs.alpha_q = update(vs.alpha_q, at[2:-2, 2:-2], self._read_var_from_nc(f"alpha_q_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    # denitrification parameters
                    vs.km_denit_rz = update(vs.km_denit_rz, at[2:-2, 2:-2], self._read_var_from_nc(f"km_denit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    vs.km_denit_ss = update(vs.km_denit_ss, at[2:-2, 2:-2], self._read_var_from_nc(f"km_denit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    # nitrification parameters
                    vs.dmax_denit_rz = update(vs.dmax_denit_rz, at[2:-2, 2:-2], self._read_var_from_nc(f"dmax_denit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    vs.dmax_denit_ss = update(vs.dmax_denit_ss, at[2:-2, 2:-2], self._read_var_from_nc(f"dmax_denit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    vs.km_nit_rz = update(vs.km_nit_rz, at[2:-2, 2:-2], self._read_var_from_nc(f"km_nit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                    vs.dmax_nit_rz = update(vs.dmax_nit_rz, at[2:-2, 2:-2], self._read_var_from_nc(f"dmax_nit_{crop}", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
        ...
        def set_initial_conditions_setup(self, state):
            ...
            vs.S_rz = update(vs.S_rz, at[2:-2, 2:-2, :vs.taup1], self._read_var_from_nc("S_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, vs.itt, npx.newaxis])
            vs.S_ss = update(vs.S_ss, at[2:-2, 2:-2, :vs.taup1], self._read_var_from_nc("S_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc')[:, :, vs.itt, npx.newaxis])
        ...
        def set_forcing_setup(self, state):
            ...
            vs.PREC_DIST_DAILY = update(vs.PREC_DIST_DAILY, at[2:-2, 2:-2, :], self._read_var_from_nc("prec", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.INF_MAT_RZ = update(vs.INF_MAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_mat_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.INF_PF_RZ = update(vs.INF_PF_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_mp_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc') + self._read_var_from_nc("inf_sc_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.INF_PF_SS = update(vs.INF_PF_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.TRANSP = update(vs.TRANSP, at[2:-2, 2:-2, :], self._read_var_from_nc("transp", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.EVAP_SOIL = update(vs.EVAP_SOIL, at[2:-2, 2:-2, :], self._read_var_from_nc("evap_soil", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.CPR_RZ = update(vs.CPR_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("cpr_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.Q_RZ = update(vs.Q_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("q_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.Q_SS = update(vs.Q_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("q_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.RE_RG = update(vs.RE_RG, at[2:-2, 2:-2, :], self._read_var_from_nc("re_rg", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.RE_RL = update(vs.RE_RL, at[2:-2, 2:-2, :], self._read_var_from_nc("re_rl", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_RZ = update(vs.S_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_rz", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_SS = update(vs.S_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_ss", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'))
            vs.S_S = update(vs.S_S, at[2:-2, 2:-2, :], vs.S_RZ[2:-2, 2:-2, :] + vs.S_SS[2:-2, 2:-2, :])
            vs.TA = update(
                vs.TA,
                at[:],
                self._read_var_from_nc(
                    "ta", self._input_dir1, f'SVATCROP_{lys_experiment}_best_simulation.nc'
                )[0, 0, :],
            )
            vs.ta_year = update(
                vs.ta_year,
