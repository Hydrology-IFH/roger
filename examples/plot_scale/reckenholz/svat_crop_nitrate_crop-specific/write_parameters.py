...
def main(nsamples):
    ...
    for lys_experiment in lys_experiments:
        ...
        idx_best = int(df_params_metrics["id"].values[0])
        df_params_metrics_best = df_params_metrics.loc[idx_best, :].to_frame().T
        df_params_best = df_params_metrics_best.loc[:, ["dmpv", "lmpv", "theta_ac", "theta_ufc", "theta_pwp", "ks", "kf", "c_canopy", "c_root", "clay"]]

        zsoil = 1350
        soil_fertility = 3
        clay = df_params_best["clay"].values.astype(onp.float64)
