...
@click.option("-td", "--tmp-dir", type=str, default=Path(__file__).parent.parent / "output" / "svat_crop_nitrate_monte_carlo_nitrogen-crop-specific")
@roger_base_cli
def main(lys_experiment, transport_model_structure, tmp_dir):
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            vs.alpha_transp_crops = update(vs.alpha_transp_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"alpha_transp_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            vs.alpha_q_crops = update(vs.alpha_q_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"alpha_q_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            vs.alpha_transp_crops = update(vs.alpha_transp_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"alpha_transp_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            vs.alpha_q_crops = update(vs.alpha_q_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"alpha_q_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
        ...
        def set_parameters(self, state):
            ...
            # partition coefficients
            vs.alpha_transp = update(vs.alpha_transp, at[2:-2, 2:-2], npx.where(mask00[2:-2, 2:-2], vs.alpha_transp_crops[2:-2, 2:-2, 0], vs.alpha_transp[2:-2, 2:-2]))
            vs.alpha_q = update(vs.alpha_q, at[2:-2, 2:-2], npx.where(mask00[2:-2, 2:-2], vs.alpha_q_crops[2:-2, 2:-2, 0], vs.alpha_q[2:-2, 2:-2]))
            # denitrification parameters
