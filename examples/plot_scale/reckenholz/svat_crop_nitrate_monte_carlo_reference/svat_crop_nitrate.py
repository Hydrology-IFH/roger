...
@click.option("-td", "--tmp-dir", type=str, default=Path(__file__).parent.parent / "output" / "svat_crop_nitrate_monte_carlo_reference")
@roger_base_cli
def main(lys_experiment, transport_model_structure, sas_solver, tmp_dir):
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        def _read_var_from_csv(self, var, path_dir, file):
            ...
            return npx.array(var_obj, dtype=rs.float_type)[:, npx.newaxis]
        ...
        def set_settings(self, state):
            ...
            settings.nx, settings.ny = self._get_nsamples(self._base_path, 'parameters.csv'), 1
        ...
        def set_parameters_setup(self, state):
            ...
            vs.alpha_transp = update(vs.alpha_transp, at[2:-2, 2:-2], self._read_var_from_csv("alpha_transp", self._base_path, "parameters.csv"))
            vs.alpha_q = update(vs.alpha_q, at[2:-2, 2:-2], self._read_var_from_csv("alpha_q", self._base_path, "parameters.csv"))
            # denitrification parameters
            vs.km_denit_rz = update(vs.km_denit_rz, at[2:-2, 2:-2], self._read_var_from_csv("km_denit", self._base_path, "parameters.csv"))
            vs.km_denit_ss = update(vs.km_denit_ss, at[2:-2, 2:-2], self._read_var_from_csv("km_denit", self._base_path, "parameters.csv"))
            vs.dmax_denit_rz = update(vs.dmax_denit_rz, at[2:-2, 2:-2], self._read_var_from_csv("dmax_denit", self._base_path, "parameters.csv"))
            vs.dmax_denit_ss = update(vs.dmax_denit_ss, at[2:-2, 2:-2], self._read_var_from_csv("dmax_denit", self._base_path, "parameters.csv"))
            # nitrification parameters
            vs.km_nit_rz = update(vs.km_nit_rz, at[2:-2, 2:-2], self._read_var_from_csv("km_nit", self._base_path, "parameters.csv"))
            vs.km_nit_ss = update(vs.km_nit_ss, at[2:-2, 2:-2], 0)
            vs.dmax_nit_rz = update(vs.dmax_nit_rz, at[2:-2, 2:-2], self._read_var_from_csv("dmax_nit", self._base_path, "parameters.csv"))
            vs.dmax_nit_ss = update(vs.dmax_nit_ss, at[2:-2, 2:-2], 0)
            # soil nitrogen mineralization parameters
            vs.kmin_rz = update(vs.kmin_rz, at[2:-2, 2:-2], self._read_var_from_csv("kmin", self._base_path, "parameters.csv"))
            vs.kmin_ss = update(vs.kmin_ss, at[2:-2, 2:-2], 0)
            # gaseous loss parameters
            vs.kngl_rz = update(vs.kngl_rz, at[2:-2, 2:-2], self._read_var_from_csv("kngl", self._base_path, "parameters.csv"))
            ...
            if settings.tm_structure == "complete-mixing":
                ...
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], 0.6)
                ...
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 4], self._read_var_from_csv("c2_transp", self._base_path, "parameters.csv"))
                ...
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 4], self._read_var_from_csv("c2_q_rz", self._base_path, "parameters.csv"))
                ...
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 4], self._read_var_from_csv("c2_q_ss", self._base_path, "parameters.csv"))
        ...
        @roger_routine
        def set_diagnostics(self, state, base_path=Path(__file__).parent.parent / "output" / "svat_crop_nitrate_monte_carlo_reference"):
    ...
    def apply_fertilizer_kernel(state):
        ...
        vs.Nfert_min = update_add(vs.Nfert_min, at[2:-2, 2:-2], vs.NMIN_IN[2:-2, 2:-2, vs.itt])
        vs.Nfert_min = update(vs.Nfert_min, at[2:-2, 2:-2], npx.where(vs.Nfert_min[2:-2, 2:-2] < 0, 0, vs.Nfert_min[2:-2, 2:-2]))

        vs.Nmin_in = update_add(vs.Nmin_in, at[2:-2, 2:-2], vs.NMIN_IN[2:-2, 2:-2, vs.itt])
        ...
        vs.Nfert_org = update_add(vs.Nfert_org , at[2:-2, 2:-2], vs.NORG_IN[2:-2, 2:-2, vs.itt])
