...
@click.option("-td", "--tmp-dir", type=str, default=Path(__file__).parent.parent / "output" / "svat_crop_nitrate_monte_carlo_semi-crop-specific")
@roger_base_cli
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        def set_parameters_setup(self, state):
            ...
            if transport_model_structure == "advection-dispersion-power":
                vs.k_transp_crops = update(vs.k_transp_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"k_transp_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.k_q_crops = update(vs.k_q_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"k_q_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.k_transp_crops = update(vs.k_transp_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"k_transp_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.k_q_crops = update(vs.k_q_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"k_q_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
            elif settings.tm_structure == "time-variant_advection-dispersion-power":
                vs.c1_transp_crops = update(vs.c1_transp_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"c1_transp_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c1_q_crops = update(vs.c1_q_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"c1_q_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c2_transp_crops = update(vs.c2_transp_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"c2_transp_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c2_q_crops = update(vs.c2_q_crops, at[2:-2, 2:-2, 0], self._read_var_from_nc(f"c2_q_crop", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c1_transp_crops = update(vs.c1_transp_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"c1_transp_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c1_q_crops = update(vs.c1_q_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"c1_q_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c2_transp_crops = update(vs.c2_transp_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"c2_transp_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
                vs.c2_q_crops = update(vs.c2_q_crops, at[2:-2, 2:-2, 1], self._read_var_from_nc(f"c2_q_bare", self._base_path, f"parameters_for_{transport_model_structure}_{lys_experiment}.nc"))
        ...
        def set_parameters(self, state):
            ...
            if settings.tm_structure == "advection-dispersion-power":
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], npx.where(mask00[2:-2, 2:-2], vs.k_transp_crops[2:-2, 2:-2, 0], vs.sas_params_transp[2:-2, 2:-2, 1]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 1], npx.where(mask00[2:-2, 2:-2], vs.k_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_rz[2:-2, 2:-2, 1]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 1], npx.where(mask00[2:-2, 2:-2], vs.k_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_ss[2:-2, 2:-2, 1]))

                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 1], npx.where(mask7[2:-2, 2:-2], vs.k_transp_crops[2:-2, 2:-2, 1], vs.sas_params_transp[2:-2, 2:-2, 1]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 1], npx.where(mask7[2:-2, 2:-2], vs.k_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_rz[2:-2, 2:-2, 1]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 1], npx.where(mask7[2:-2, 2:-2], vs.k_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_ss[2:-2, 2:-2, 1]))


            elif settings.tm_structure == "time-variant_advection-dispersion-power":
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 3], npx.where(mask00[2:-2, 2:-2], vs.c1_transp_crops[2:-2, 2:-2, 0], vs.sas_params_transp[2:-2, 2:-2, 3]))
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 4], npx.where(mask00[2:-2, 2:-2], vs.c2_transp_crops[2:-2, 2:-2, 0], vs.sas_params_transp[2:-2, 2:-2, 4]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 3], npx.where(mask00[2:-2, 2:-2], vs.c1_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_rz[2:-2, 2:-2, 3]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 4], npx.where(mask00[2:-2, 2:-2], vs.c2_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_rz[2:-2, 2:-2, 4]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 3], npx.where(mask00[2:-2, 2:-2], vs.c1_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_ss[2:-2, 2:-2, 3]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 4], npx.where(mask00[2:-2, 2:-2], vs.c2_q_crops[2:-2, 2:-2, 0], vs.sas_params_q_ss[2:-2, 2:-2, 4]))

                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 3], npx.where(mask7[2:-2, 2:-2], vs.c1_transp_crops[2:-2, 2:-2, 1], vs.sas_params_transp[2:-2, 2:-2, 3]))
                vs.sas_params_transp = update(vs.sas_params_transp, at[2:-2, 2:-2, 4], npx.where(mask7[2:-2, 2:-2], vs.c2_transp_crops[2:-2, 2:-2, 1], vs.sas_params_transp[2:-2, 2:-2, 4]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 3], npx.where(mask7[2:-2, 2:-2], vs.c1_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_rz[2:-2, 2:-2, 3]))
                vs.sas_params_q_rz = update(vs.sas_params_q_rz, at[2:-2, 2:-2, 4], npx.where(mask7[2:-2, 2:-2], vs.c2_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_rz[2:-2, 2:-2, 4]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 3], npx.where(mask7[2:-2, 2:-2], vs.c1_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_ss[2:-2, 2:-2, 3]))
                vs.sas_params_q_ss = update(vs.sas_params_q_ss, at[2:-2, 2:-2, 4], npx.where(mask7[2:-2, 2:-2], vs.c2_q_crops[2:-2, 2:-2, 1], vs.sas_params_q_ss[2:-2, 2:-2, 4]))
