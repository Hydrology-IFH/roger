...
def main(tmp_dir):
    ...
    base_path_output = base_path / "output" / "svat_crop_nitrate_monte_carlo_nitrogen"
    if not os.path.exists(base_path_output):
        os.mkdir(base_path_output)
    # directory of figures
    base_path_figs = Path(__file__).parent.parent / "figures" / "svat_crop_nitrate_monte_carlo_nitrogen"
    ...
    for i, lys_experiment in enumerate(lys_experiments):
        ...
        ds_obs = ds_obs.assign_coords(date=("Time", date_obs))
        obs_vals = ds_obs["NMIN_IN"].isel(x=0, y=0).values * 0.5
        df_obs = pd.DataFrame(index=date_obs, columns=["obs"])
        df_obs.loc[:, "obs"] = obs_vals
