...
@click.option("-td", "--tmp-dir", type=str, default=Path(__file__).parent.parent / "output" / "svat_crop_monte_carlo_crop-specific")
@roger_base_cli
def main(lys_experiment, tmp_dir):
    ...
    class SVATCROPSetup(RogerSetup):
        ...
        _input_dir = _base_path.parent / "input" / f"{lys_experiment}"
        ...
        def set_settings(self, state):
            ...
            settings.identifier = f'SVATCROP_{lys_experiment}'
        ...
        def set_parameters_setup(self, state):
            ...
            vs.root_growth_scale = update(vs.root_growth_scale, at[2:-2, 2:-2], self._read_var_from_csv("c_root_winter-barley", self._base_path, "parameters.csv"))
            vs.canopy_growth_scale = update(vs.canopy_growth_scale, at[2:-2, 2:-2], self._read_var_from_csv("c_canopy_winter-barley", self._base_path, "parameters.csv"))
            vs.basal_crop_coeff_scale = update(vs.basal_crop_coeff_scale, at[2:-2, 2:-2], self._read_var_from_csv("c_canopy_winter-barley", self._base_path, "parameters.csv"))
        ...
        def set_parameters(self, state):
            ...
            for crop_type in crop_types:
                if (vs.lu_id[2:-2, 2:-2] == crop_type).all():
                    crop_name = self._dict_crops[crop_type]
                    vs.root_growth_scale = update(vs.root_growth_scale, at[2:-2, 2:-2], self._read_var_from_csv(f"c_root_{crop_name}", self._base_path, "parameters.csv"))
                    vs.canopy_growth_scale = update(vs.canopy_growth_scale, at[2:-2, 2:-2], self._read_var_from_csv(f"c_canopy_{crop_name}", self._base_path, "parameters.csv"))
                    vs.basal_crop_coeff_scale = update(vs.basal_crop_coeff_scale, at[2:-2, 2:-2], self._read_var_from_csv(f"c_canopy_{crop_name}", self._base_path, "parameters.csv"))

                elif (vs.lu_id[2:-2, 2:-2] == 599).all():
                    vs.root_growth_scale = update(vs.root_growth_scale, at[2:-2, 2:-2], 0)
                    vs.canopy_growth_scale = update(vs.canopy_growth_scale, at[2:-2, 2:-2], 0)
                    vs.basal_crop_coeff_scale = update(vs.basal_crop_coeff_scale, at[2:-2, 2:-2], 0)
        ...
        def set_diagnostics(self, state, base_path=tmp_dir):
            ...
            diagnostics["rate"].output_variables = ["prec", "aet", "transp", "evap_soil", "inf_mat_rz", "inf_mp_rz", "inf_sc_rz", "inf_ss", "q_rz", "q_ss", "cpr_rz", "re_rg", "re_rl"]
