...
def main(nsamples):
    ...
    for transport_model_structure in ['complete-mixing', 'advection-dispersion-power', 'time-variant_advection-dispersion-power']:
        ...
        df_params = pd.DataFrame(index=range(1))
        ...
        if transport_model_structure == 'advection-dispersion-power':
            df_params.loc[:, "k_transp"] = _VALS["k_transp"]
            df_params.loc[:, "k_q_rz"] = _VALS["k_q_rz"]
            df_params.loc[:, "k_q_ss"] = _VALS["k_q_ss"]
        ...
        with h5netcdf.File(params_file, "w", decode_vlen_strings=False) as f:
            ...
            for i, key in enumerate(param_names):
                ...
                vals = df_params.iloc[:, i].values.flatten().reshape(1, 1)
                v[:, :] = vals
