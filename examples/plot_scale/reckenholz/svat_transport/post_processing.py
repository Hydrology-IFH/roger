...
for lys_experiment in lys_experiments:
    for tm_structure in tm_structures:
        ...
        base_path_results = base_path / "results" / lys_experiment / tm_structure
        if not os.path.exists(base_path_results):
            os.mkdir(base_path_results)
        # directory of figures
        base_path_figs = base_path / "figures" / lys_experiment / tm_structure
        ...
        df_perc_Br_sim['Br'] = ds_sim_tm['C_q_ss'].isel(x=0, y=0).values
        ...
        with h5netcdf.File(states_tm_file, 'a', decode_vlen_strings=False) as f:
            v = f.groups[lys_experiment].groups[tm_structure].create_variable('C_NO3_perc_cs', ('x', 'y', 'Time'), float)
            v[:, :, :] = Br_perc_cs
            v.attrs.update(long_name="composite sample of bromide concentration in percolation",
                           units="mg/l")
            v = f.groups[lys_experiment].groups[tm_structure].create_variable('M_NO3_perc_cs', ('x', 'y', 'Time'), float)
            v[:, :, :] = Br_mass_perc_cs
...
for lys_experiment in lys_experiments:
    for tm_structure in tm_structures:
        ...
        with h5netcdf.File(states_tm_file, 'a', decode_vlen_strings=False) as f:
            v = f.groups[lys_experiment].groups[tm_structure].create_variable('C_NO3_perc_cs', ('x', 'y', 'Time'), float)
            v[:, :, :] = NO3_perc_cs
            v.attrs.update(long_name="composite sample of nitrate-nitrogen concentration in percolation",
                           units="mg/l")
            v = f.groups[lys_experiment].groups[tm_structure].create_variable('M_NO3_perc_cs', ('x', 'y', 'Time'), float)
            v[:, :, :] = NO3_mass_perc_cs
            v.attrs.update(long_name="composite sample of nitrate-nitrogen mass in percolation",
                           units="mg")
