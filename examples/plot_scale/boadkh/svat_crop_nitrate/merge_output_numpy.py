        ...
        for crop_rotation_scenario in crop_rotation_scenarios:
            for fertilization_intensity in fertilization_intensities:
                path = str(
                    base_path
                    / "output"
                    / "svat_crop_nitrate_debug"
                    / f"SVATCROPNITRATE_{location}_{crop_rotation_scenario}_{fertilization_intensity}_Nfert_numpy.*.nc"
                )
                output_tm_file = (
                    base_path
                    / "output"
                        ...
                        for dfs in diag_files:
                            with h5netcdf.File(dfs, "r", decode_vlen_strings=False) as df:
                                ...
                                for var_sim in list(df.variables.keys()):
                                    _file = base_path / "output" / "svat_crop_nitrate_debug" / f"SVATCROPNITRATE_{location}_{crop_rotation_scenario}_{fertilization_intensity}_Nfert.collect.nc"
