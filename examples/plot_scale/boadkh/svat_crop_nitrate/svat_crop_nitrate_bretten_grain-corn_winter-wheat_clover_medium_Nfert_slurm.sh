...
#SBATCH --job-name=svat_crop_nitrate_bretten_grain-corn_winter-wheat_clover_medium_Nfert
#SBATCH --output=svat_crop_nitrate_bretten_grain-corn_winter-wheat_clover_medium_Nfert.out
#SBATCH --error=svat_crop_nitrate_bretten_grain-corn_winter-wheat_clover_medium_Nfert_err.out
#SBATCH --export=ALL
...
# Compares hashes
checksum_gws=$(shasum -a 256 /pfs/work7/workspace/scratch/fr_rs1092-workspace1/roger/examples/plot_scale/boadkh/output/svat_crop/SVATCROP_bretten_grain-corn_winter-wheat_clover.nc | cut -f 1 -d " ")
checksum_ssd=0a
cp /pfs/work7/workspace/scratch/fr_rs1092-workspace1/roger/examples/plot_scale/boadkh/output/svat_crop/SVATCROP_bretten_grain-corn_winter-wheat_clover.nc "${TMPDIR}"
# Wait for termination of moving files
while [ "${checksum_gws}" != "${checksum_ssd}" ]; do
sleep 10
checksum_ssd=$(shasum -a 256 "${TMPDIR}"/SVATCROP_bretten_grain-corn_winter-wheat_clover.nc | cut -f 1 -d " ")
done
