...
@click.option("-nr", "--nrepeat", type=int, default=3)
@click.command("main")
def main(nrepeat):
    ...
    for location in locations:
        for crop_rotation_scenario in crop_rotation_scenarios:
            ...
            input_file = base_path.parent / "output" / "svat_crop" / f"SVATCROP_{location}_{crop_rotation_scenario}_for_SVATCROPNITRATE.nc"
            if not os.path.exists(input_file):
                with h5netcdf.File(input_file, "w", decode_vlen_strings=False) as f:
                    f.attrs.update(
                        date_created=datetime.datetime.today().isoformat(),
                        title=f"RoGeR simulations at {location} and with {crop_rotation_scenario1} for crop rotation",
