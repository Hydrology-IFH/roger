...
def main(location, crop_rotation_scenario, fertilization_intensity, tmp_dir):
    ...
    class SVATCROPNITRATESetup(RogerSetup):
        ...
        def set_settings(self, state):
            ...
            settings.identifier = f"SVATCROPNITRATE_{self._id}_{location}_{crop_rotation_scenario}"
            settings.sas_solver = "deterministic"
            settings.sas_solver_substeps = 6

            settings.nx, settings.ny = 3, 1
            settings.nitt = self._get_nitt(
                self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
            )
        ...
        def set_parameters_setup(self, state):
            ...
            vs.S_PWP_RZ = update(
                ...
                self._read_var_from_nc(
                    "S_pwp_rz", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.S_SAT_RZ = update(
                vs.S_SAT_RZ,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "S_sat_rz", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.S_PWP_SS = update(
                vs.S_PWP_SS,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "S_pwp_ss", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.S_SAT_SS = update(
                vs.S_SAT_SS,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "S_sat_ss", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            ...
            vs.soil_fertility = update(
                vs.soil_fertility, at[2:-2, 2:-2], 2
            )

            # soil temperature parameters
            vs.z_soil = update(
                vs.z_soil, at[2:-2, 2:-2], self._read_var_from_nc("z_soil", self._base_path, "z_soil.nc")[self._x1, self._y1]
            )
            vs.phi_soil_temp = update(vs.phi_soil_temp, at[2:-2, 2:-2], 91)
            # dampening depth of soil temperature depends on clay content
            clay = self._read_var_from_nc("clay", self._base_path, "clay.nc")[self._x1, self._y1]
            ...
            vs.YEAR = update(
                ...
                at[:],
                self._read_var_from_nc(
                    "year", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.DOY = update(
                vs.DOY,
                at[:],
                self._read_var_from_nc(
                    "doy", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.LU_ID = update(
                vs.LU_ID,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "lu_id", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                )[self._y1, self._x1, :],
            )
            vs.Z_ROOT = update(
                vs.Z_ROOT,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "z_root", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                )[self._y1, self._x1, :],
            )
        ...
        def set_initial_conditions_setup(self, state):
            ...
            vs.S_rz = update(
                ...
                at[2:-2, 2:-2, :vs.taup1],
                self._read_var_from_nc(
                    "S_rz", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                )[:, :, vs.itt, npx.newaxis],
            )
            vs.S_ss = update(
                vs.S_ss,
                at[2:-2, 2:-2, :vs.taup1],
                self._read_var_from_nc(
                    "S_ss", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                )[:, :, vs.itt, npx.newaxis],
            )
            vs.S_s = update(
        ...
        def set_forcing_setup(self, state):
            ...
            vs.PREC_DIST_DAILY = update(
                ...
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "prec", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.INF_MAT_RZ = update(
                ...
                self._read_var_from_nc(
                    "inf_mat_rz",
                    self._input_dir,
                    f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc",
                ),
            )
            vs.INF_PF_RZ = update(
                vs.INF_PF_RZ,
            ...
            vs.INF_PF_SS = update(
                ...
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "inf_ss", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.TRANSP = update(
                vs.TRANSP,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "transp", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.EVAP_SOIL = update(
                vs.EVAP_SOIL,
            ...
            vs.CPR_RZ = update(
                ...
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "cpr_rz", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.Q_RZ = update(
                vs.Q_RZ,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "q_rz", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.Q_SS = update(
                vs.Q_SS,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "q_ss", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.RE_RG = update(
                vs.RE_RG,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "re_rg", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.RE_RL = update(
                vs.RE_RL,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc(
                    "re_rl", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                ),
            )
            vs.S_RZ = update(
                vs.S_RZ,
            ...
            vs.TA = update(
                ...
                at[:],
                self._read_var_from_nc(
                    "ta", self._input_dir, f"SVATCROP_{self._id}_{location}_{crop_rotation_scenario}_3_grid_cells.nc"
                )[0, 0, :],
            )
            vs.ta_year = update(
                vs.ta_year,
        ...
        def set_diagnostics(self, state, base_path=tmp_dir):
            ...
            diagnostics["rate"].output_variables = ["M_in", "M_q_ss", "M_transp", "Nmin_in"]
