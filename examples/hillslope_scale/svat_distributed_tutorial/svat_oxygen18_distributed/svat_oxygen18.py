...
def main(tmp_dir):
    ...
    class SVATTRANSPORTSetup(RogerSetup):
        ...
        def set_settings(self, state):
            ...
            settings.identifier = (
                f'SVATOXYGEN18_{self._config["TRANSPORT_MODEL_STRUCTURE"]}_{self._config["SAS_SOLVER"]}'
            )
        ...
        def set_initial_conditions_setup(self, state):
            ...
            vs.S_snow = update(
                vs.S_snow,
                at[2:-2, 2:-2, : vs.taup1],
                self._read_var_from_nc("S_snow", self._base_path, "states_hm.nc")[:, :, vs.itt, npx.newaxis],
            )
            vs.S_rz = update(
                vs.S_rz,
                at[2:-2, 2:-2, : vs.taup1],
                self._read_var_from_nc("S_rz", self._base_path, "states_hm.nc")[:, :, vs.itt, npx.newaxis],
            )
            vs.S_ss = update(
                vs.S_ss,
                at[2:-2, 2:-2, : vs.taup1],
                self._read_var_from_nc("S_ss", self._base_path, "states_hm.nc")[:, :, vs.itt, npx.newaxis],
            )
            vs.S_s = update(
        ...
        def set_forcing_setup(self, state):
            ...
            vs.PREC_DIST_DAILY = update(
                vs.PREC_DIST_DAILY, at[2:-2, 2:-2, :], self._read_var_from_nc("prec", self._base_path, "states_hm.nc")
            )
            vs.INF_MAT_RZ = update(
                vs.INF_MAT_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_mat_rz", self._base_path, "states_hm.nc")
            )
            vs.INF_PF_RZ = update(
                vs.INF_PF_RZ,
                at[2:-2, 2:-2, :],
                self._read_var_from_nc("inf_mp_rz", self._base_path, "states_hm.nc")
                + self._read_var_from_nc("inf_sc_rz", self._base_path, "states_hm.nc"),
            )
            vs.INF_PF_SS = update(
                vs.INF_PF_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("inf_ss", self._base_path, "states_hm.nc")
            )
            vs.TRANSP = update(
                vs.TRANSP, at[2:-2, 2:-2, :], self._read_var_from_nc("transp", self._base_path, "states_hm.nc")
            )
            vs.EVAP_SOIL = update(
                vs.EVAP_SOIL, at[2:-2, 2:-2, :], self._read_var_from_nc("evap_soil", self._base_path, "states_hm.nc")
            )
            vs.CPR_RZ = update(
                vs.CPR_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("cpr_rz", self._base_path, "states_hm.nc")
            )
            vs.Q_RZ = update(
                vs.Q_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("q_rz", self._base_path, "states_hm.nc")
            )
            vs.Q_SS = update(
                vs.Q_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("q_ss", self._base_path, "states_hm.nc")
            )
            vs.S_RZ = update(
                vs.S_RZ, at[2:-2, 2:-2, :], self._read_var_from_nc("S_rz", self._base_path, "states_hm.nc")
            )
            vs.S_SS = update(
                vs.S_SS, at[2:-2, 2:-2, :], self._read_var_from_nc("S_ss", self._base_path, "states_hm.nc")
            )
            vs.S_S = update(vs.S_S, at[2:-2, 2:-2, :], vs.S_RZ[2:-2, 2:-2, :] + vs.S_SS[2:-2, 2:-2, :])
            vs.S_SNOW = update(
                vs.S_SNOW, at[2:-2, 2:-2, :], self._read_var_from_nc("S_snow", self._base_path, "states_hm.nc")
            )
