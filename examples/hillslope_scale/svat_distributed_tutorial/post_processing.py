...
axl1 = fig.add_axes([0.8, 0.59, 0.02, 0.29])
...
axes[1, 0].imshow(ds_tm['tt50_q_ss'].isel(Time=120).values.T, origin="lower", cmap='Purples', vmin=100, vmax=400)
...
axes[1, 1].imshow(ds_tm['tt50_q_ss'].isel(Time=300).values.T, origin="lower", cmap='Purples', vmin=100, vmax=400)
...
cmap = copy.copy(plt.cm.get_cmap('Purples'))
norm = mpl.colors.Normalize(vmin=100, vmax=400)
axl2 = fig.add_axes([0.8, 0.12, 0.02, 0.29])
