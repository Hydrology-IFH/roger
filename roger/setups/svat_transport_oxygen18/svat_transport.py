...
class SVATTRANSPORTSetup(RogerSetup):
    ...
    def set_forcing_setup(self, state):
        ...
        if settings.enable_deuterium:
            vs.C_IN = update(vs.C_IN, at[2:-2, 2:-2, :], self._read_var_from_nc("d2H", self._base_path, 'forcing_tracer.nc'))

        if settings.enable_oxygen18:
            vs.C_IN = update(vs.C_IN, at[2:-2, 2:-2, :], self._read_var_from_nc("d18O", self._base_path, 'forcing_tracer.nc'))
