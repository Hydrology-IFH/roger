...
class DISTGROUNDWATERTRANSPORTSetup(RogerSetup):
    ...
    def set_parameters(self, state):
        ...
        if settings.tm_structure == "complete-mixing":
            ...
            vs.sas_params_q_re = update(vs.sas_params_q_re, at[:, :, 0], 1)
            vs.sas_params_q_gw = update(vs.sas_params_q_gw, at[:, :, 0], 1)
            vs.sas_params_q_leak = update(vs.sas_params_q_leak, at[:, :, 0], 1)
            ...
            vs.sas_params_q_re = update(vs.sas_params_q_re, at[:, :, 0], 22)
            vs.sas_params_q_gw = update(vs.sas_params_q_gw, at[:, :, 0], 22)
            vs.sas_params_q_leak = update(vs.sas_params_q_leak, at[:, :, 0], 22)
            ...
            vs.sas_params_q_re = update(vs.sas_params_q_re, at[:, :, 0], self._read_var_from_nc("sas_function", 'sas_parameters_q_re.nc'))
            vs.sas_params_q_re = update(vs.sas_params_q_re, at[:, :, 1], self._read_var_from_nc("a", 'sas_parameters_q_re.nc'))
            vs.sas_params_q_re = update(vs.sas_params_q_re, at[:, :, 2], self._read_var_from_nc("b", 'sas_parameters_q_re.nc'))
            vs.sas_params_q_gw = update(vs.sas_params_q_gw, at[:, :, 0], self._read_var_from_nc("sas_function", 'sas_parameters_q_gw.nc'))
            vs.sas_params_q_gw = update(vs.sas_params_q_gw, at[:, :, 1], self._read_var_from_nc("a", 'sas_parameters_q_gw.nc'))
            vs.sas_params_q_gw = update(vs.sas_params_q_gw, at[:, :, 2], self._read_var_from_nc("b", 'sas_parameters_q_gw.nc'))
            vs.sas_params_q_leak = update(vs.sas_params_q_leak, at[:, :, 0], self._read_var_from_nc("sas_function", 'sas_parameters_q_leak.nc'))
            vs.sas_params_q_leak = update(vs.sas_params_q_leak, at[:, :, 1], self._read_var_from_nc("a", 'sas_parameters_q_leak.nc'))
            vs.sas_params_q_leak = update(vs.sas_params_q_leak, at[:, :, 2], self._read_var_from_nc("b", 'sas_parameters_q_leak.nc'))
    ...
    def set_initial_conditions(self, state):
        ...
        vs.S_VAD = update(vs.S_VAD, at[:, :, :], self._read_var_from_nc("S_gw", 'states_hm.nc'))
        vs.S_GW = update(vs.S_GW, at[:, :, :], self._read_var_from_nc("S_gw", 'states_hm.nc'))
        ...
        vs.S_vad = update(vs.S_vad, at[:, :, :2], vs.S_VAD[:, :, 0, npx.newaxis])
        vs.S_gw = update(vs.S_gw, at[:, :, :2], vs.S_GW[:, :, 0, npx.newaxis])
        ...
        vs.sa_vad = update(
            vs.sa_vad,
            at[:, :, :2, 1:], npx.diff(npx.linspace(arr0, vs.S_vad[:, :, vs.tau], settings.ages, axis=-1), axis=-1)[:, :, npx.newaxis, :],
        )
        vs.SA_vad = update(
            vs.SA_vad,
            at[:, :, :, 1:], npx.cumsum(vs.sa_vad, axis=3),
        )
        vs.sa_gw = update(
            vs.sa_gw,
            at[:, :, :2, 1:], npx.diff(npx.linspace(arr0, vs.S_gw[:, :, vs.tau], settings.ages, axis=-1), axis=-1)[:, :, npx.newaxis, :],
        )
        vs.SA_gw = update(
            vs.SA_gw,
            at[:, :, :, 1:], npx.cumsum(vs.sa_gw, axis=3),
        )
        ...
        if (settings.enable_bromide | settings.enable_chloride):
            ...
            vs.C_vad = update(vs.C_vad, at[:, :, :2], self._read_var_from_nc("C_vad", 'initvals.nc')[:, :, npx.newaxis])
            vs.C_gw = update(vs.C_gw, at[:, :, :2], self._read_var_from_nc("C_gw", 'initvals.nc')[:, :, npx.newaxis])
            vs.msa_rz = update(
                vs.msa_rz,
            ...
            vs.msa_vad = update(
                vs.msa_vad,
                at[:, :, :2, :], vs.C_vad[:, :, :2, npx.newaxis] * vs.sa_vad[:, :, :2, :],
            )
            vs.M_vad = update(
                vs.M_vad,
                at[:, :, :], npx.sum(vs.msa_vad, axis=-1),
            )
            vs.msa_gw = update(
                vs.msa_gw,
                at[:, :, :2, :], vs.C_gw[:, :, :2, npx.newaxis] * vs.sa_gw[:, :, :2, :],
            )
            vs.M_gw = update(
                vs.M_gw,
                at[:, :, :], npx.sum(vs.msa_gw, axis=-1),
            )
    ...
    def set_forcing(self, state):
        ...
        if (vs.itt == 0):
            ...
            vs.CPR_SS = update(vs.CPR_SS, at[:, :, :], self._read_var_from_nc("cpr_ss", 'states_hm.nc'))
            ...
            vs.Q_RE = update(vs.Q_RE, at[:, :, :], self._read_var_from_nc("q_re", 'states_hm.nc'))
            vs.Q_GW = update(vs.Q_GW, at[:, :, :], self._read_var_from_nc("q_gw", 'states_hm.nc'))
            vs.Q_LEAK = update(vs.Q_LEAK, at[:, :, :], self._read_var_from_nc("q_leak", 'states_hm.nc'))
...
def set_states_kernel(state):
    ...
    vs.cpr_ss = update(vs.cpr_ss, at[:, :], vs.CPR_SS[:, :, vs.itt])
    vs.q_sub_rz = update(vs.q_sub_rz, at[:, :], vs.Q_SUB_RZ[:, :, vs.itt])
    vs.q_sub_ss = update(vs.q_sub_ss, at[:, :], vs.Q_SUB_SS[:, :, vs.itt])
    vs.q_re = update(vs.q_re, at[:, :], vs.Q_RE[:, :, vs.itt])
    vs.q_gw = update(vs.q_gw, at[:, :], vs.Q_GW[:, :, vs.itt])
    vs.q_leak = update(vs.q_leak, at[:, :], vs.Q_LEAK[:, :, vs.itt])
    ...
    vs.S_vad = update(vs.S_vad, at[:, :, vs.tau], vs.S_VAD[:, :, vs.itt])
    vs.S_gw = update(vs.S_gw, at[:, :, vs.tau], vs.S_GW[:, :, vs.itt])
...
def after_timestep_kernel(state):
    ...
    vs.M_vad = update(
        vs.M_vad,
        at[:, :, vs.taum1], vs.M_vad[:, :, vs.tau],
    )
    vs.C_vad = update(
        vs.C_vad,
        at[:, :, vs.taum1], vs.C_vad[:, :, vs.tau],
    )
    vs.SA_gw = update(
        vs.SA_gw,
        at[:, :, vs.taum1, :], vs.SA_gw[:, :, vs.tau, :],
    )
    vs.sa_gw = update(
        vs.sa_gw,
        at[:, :, vs.taum1, :], vs.sa_gw[:, :, vs.tau, :],
    ...
    vs.M_gw = update(
        vs.M_gw,
        at[:, :, vs.taum1], vs.M_gw[:, :, vs.tau],
    )
    vs.C_gw = update(
        vs.C_gw,
        at[:, :, vs.taum1], vs.C_gw[:, :, vs.tau],
    )
...
model = DISTGROUNDWATERTRANSPORTSetup()
model.setup()
model.warmup()
model.run()
