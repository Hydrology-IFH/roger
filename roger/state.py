...
class RogerSettings(Lockable, StrictContainer):
...
class RogerVariables(Lockable, StrictContainer):
...
class DistSafeVariableWrapper(RogerVariables):
    ...
    def __getattr__(self, attr):
        ...
        if attr in orig_getattr("__metadata__") and attr not in orig_getattr("__local_variables__"):
            raise RuntimeError(
                ...
                "Consider adding it to the local_variables argument of @roger_routine."
            )
    ...
    def _scatter_variables(self):
        from roger.distributed import scatter, barrier
...
class RogerState:
    ...
    def __init__(self, var_meta, setting_meta, dimensions, diagnostics=None, plugin_interfaces=None):
        ...
        self._settings = RogerSettings(setting_meta)
    ...
    def initialize_variables(self):
        ...
        self._variables = RogerVariables(self._var_meta, self._manifest_dimensions())
...
def get_default_state(use_plugins=None):
    ...
    return RogerState(var_meta, default_settings, default_dimensions, plugin_interfaces=plugin_interfaces)


def roger_state_pytree_flatten(state):
...
def roger_state_pytree_unflatten(aux_data, leaves):
    assert len(leaves) == 1
    variables = leaves[0]

    # by-pass __init__ and set attributes manually
    state = RogerState.__new__(RogerState)
...
def roger_variables_pytree_flatten(variables):
    aux_attrs = (
        "__dimensions__",
        "__metadata__",
        "__fields__",
...
def roger_variables_pytree_unflatten(aux_data, leaves):
    keys, aux_attrs = aux_data

    # by-pass __init__ and set attributes manually
    variables = RogerVariables.__new__(RogerVariables)
...
def resize_dimension(state, dimension, new_size):
    """Resize a dimension of an existing RogerState object.
