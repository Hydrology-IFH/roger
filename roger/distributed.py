...
def validate_decomposition(dimensions):
    nx, ny = dimensions["x"], dimensions["y"]
...
def _gather_1d(nx, ny, arr, dim):
    ...
    dim_grid = ["x" if dim == 0 else "y"] + [None] * (arr.ndim - 1)
...
def _scatter_1d(nx, ny, arr, dim):
    ...
    arr = exchange_overlap(arr, ["x" if dim == 0 else "y"])
