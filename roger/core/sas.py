...
def exponential(state, SA, sas_params):
    ...
    Omega = update(
        ...
        at[2:-2, 2:-2, :], npx.where(mask51[2:-2, 2:-2, :], npx.where(SA[2:-2, 2:-2, vs.tau, :] > 0, npx.where(SA[2:-2, 2:-2, vs.tau, :] < S[2:-2, 2:-2, :],
                               1 - npx.exp(sas_params[2:-2, 2:-2, 1, npx.newaxis] * (-1) * SA[2:-2, 2:-2, vs.tau, :]), 1.), 0.), Omega[2:-2, 2:-2, :]) * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
    Omega = update(
        Omega,
        at[2:-2, 2:-2, :], npx.where(mask52[2:-2, 2:-2, :], npx.where(SA[2:-2, 2:-2, vs.tau, :] > 0, npx.where(SA[2:-2, 2:-2, vs.tau, :] < S[2:-2, 2:-2, :],
                               1 - npx.exp(sas_params[2:-2, 2:-2, 1, npx.newaxis] * (-1) * SA[2:-2, 2:-2, vs.tau, :]), 1.), 0.)[..., ::-1], Omega[2:-2, 2:-2, :]) * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
...
def power(state, SA, sas_params):
    ...
    Omega = update(
        Omega,
        at[2:-2, 2:-2, :], npx.where(mask61[2:-2, 2:-2, :], npx.where(SA[2:-2, 2:-2, vs.tau, :] > 0, npx.where(SA[2:-2, 2:-2, vs.tau, :] <= S[2:-2, 2:-2, :] * (1/sas_params[2:-2, 2:-2, 1, npx.newaxis]),
                               sas_params[2:-2, 2:-2, 1, npx.newaxis] * (SA[2:-2, 2:-2, vs.tau, :] / S[2:-2, 2:-2, :])**sas_params[2:-2, 2:-2, 2, npx.newaxis], 1.), 0.), Omega[2:-2, 2:-2, :]) * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
    Omega = update(
        Omega,
        at[2:-2, 2:-2, :], npx.where(mask62[2:-2, 2:-2, :], npx.where(SA[2:-2, 2:-2, vs.tau, :] > 0, npx.where(SA[2:-2, 2:-2, vs.tau, :] <= S[2:-2, 2:-2, :] * (1/sas_params[2:-2, 2:-2, 1, npx.newaxis]),
                               sas_params[2:-2, 2:-2, 1, npx.newaxis] * (SA[2:-2, 2:-2, vs.tau, :] / S[2:-2, 2:-2, :])**sas_params[2:-2, 2:-2, 2, npx.newaxis], 1.), 0.)[..., ::-1], Omega[2:-2, 2:-2, :]) * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
