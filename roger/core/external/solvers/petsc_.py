...
from roger import logger, roger_kernel, runtime_settings as rs, runtime_state as rst
from roger.core import utilities
from roger.core.external.solvers.base import LinearSolver
from roger.core.operators import numpy as npx, update, update_add, at, flush
...
@roger_kernel
def prepare_solver_inputs(state, z, x0, boundary_val, boundary_mask, boundary_fac):
    settings = state.settings

    if boundary_val is None:
        boundary_val = x0

    x0 = utilities.enforce_boundaries(x0)

    z = npx.where(boundary_mask, z, boundary_val)  # set right hand side on boundaries

    if settings.enable_groundwater:
        # add dirichlet BC to groundwater head
        if rst.proc_idx[0] == rs.num_proc[0] - 1:
            z = update_add(z, at[-3, 2:-2], -z[-2, 2:-2] * boundary_fac["east"])

        if rst.proc_idx[0] == 0:
            z = update_add(z, at[2, 2:-2], -z[1, 2:-2] * boundary_fac["west"])

        if rst.proc_idx[1] == rs.num_proc[1] - 1:
            z = update_add(z, at[2:-2, -3], -z[2:-2, -2] * boundary_fac["north"])

        if rst.proc_idx[1] == 0:
            z = update_add(z, at[2:-2, 2], -z[2:-2, 1] * boundary_fac["south"])
