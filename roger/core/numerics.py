...
def validate_parameters_soil(state):
    ...
    mask4 = (vs.z_soil <= 0) & vs.maskCatch
    if global_and(npx.any(mask4[2:-2, 2:-2])):
        raise ValueError("z_soil-parameter contains zero or negative values.")
    
    mask5 = (vs.z_gw[:, :, vs.tau] * 1000 < vs.z_soil) & vs.maskCatch
    if global_and(npx.any(mask5[2:-2, 2:-2])):
        raise ValueError("One or more z_gw-parameter value are less than z_soil-parameter value.")
    
    mask6 = (vs.theta_ac <= 0) & vs.maskCatch
    if global_and(npx.any(mask6[2:-2, 2:-2])):
        raise ValueError("theta_ac-parameter contains zero or negative values.")
    
    mask7 = (vs.theta_ufc <= 0) & vs.maskCatch
    if global_and(npx.any(mask7[2:-2, 2:-2])):
        raise ValueError("theta_ufc-parameter contains zero or negative values.")
    
    mask8 = (vs.theta_pwp <= 0) & vs.maskCatch
    if global_and(npx.any(mask8[2:-2, 2:-2])):
        raise ValueError("theta_pwp-parameter contains zero or negative values.")
    
    mask9 = (vs.lmpv > vs.z_soil) & vs.maskCatch
    if global_and(npx.any(mask9[2:-2, 2:-2])):
        raise ValueError("One or more lmpv-parameter values are greater than z_soil-parameter values.")
