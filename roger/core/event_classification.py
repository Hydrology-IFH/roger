...
def import_meteo_input(path_to_dir: Path):
    """Importing the meteorological data
...
def event_classification(pta, hpi=5, end_prec_event=36, sf=3, ta_fm=0, dt=6):
    ...
    df_event.loc[:, 'rain_'] = np.NaN
    df_event.loc[:, 'snow_'] = np.NaN
    df_event.loc[:, 'snow_melt_'] = np.NaN
    df_event.loc[:, 'event_'] = np.NaN
    ...
    if len(start_snow_melt) > 1:
        for i in range(1, len(start_snow_melt)):
            ...
            csum_melt = np.cumsum(df_event.loc[df_event.index[start_snow_melt[i]]:, 'TA'] * 3 * 1/6)
            end = np.where(csum_melt > sum_snow)[0][0]
            if end:
    ...
    df_event.loc[:, 'event_no'] = np.NaN
    mask = (df_event['start_event_'] == True)
    df_event.loc[mask, 'event_no'] = range(1, np.sum(mask) + 1)
    ...
    event_nos = np.unique(df_event['event_no'].values).tolist()[1:]
...
def make_variable_time_index(prec_event, enable_film_flow=False):
    ...
    event_nos_zeros = np.unique(prec_event['event_no'].values)
    event_nos_trim = np.trim_zeros(event_nos_zeros)
    event_nos = np.unique(event_nos_trim)
    ...
    for i, eno in enumerate(event_nos):
        ...
        event_type = np.min(prec_eno['event_type'].values)
    ...
    for i, d_cond in enumerate(prec_daily_event.index):
        ...
        new_prec_h['event_no'].replace(0, np.nan, inplace=True)
        new_prec_h['event_type'].replace(0, np.nan, inplace=True)

        new_prec_h['event_no'] = new_prec_h['event_no'].replace(0, np.nan)
        new_prec_h['event_type'] = new_prec_h['event_type'].replace(0, np.nan)

        new_prec_h['event_no'].fillna(method='ffill', inplace=True)
        new_prec_h['event_type'].fillna(method='ffill', inplace=True)

        new_prec_h['event_no'].replace(np.nan, 0, inplace=True)
        new_prec_h['event_type'].replace(np.nan, 0, inplace=True)
...
def calc_event_classification(input_dir, hpi=5, end_prec_event=36, sf=3, ta_fm=0,
    ...
    """
    # import meteo data
    df_PREC, df_PET, df_Ta = import_meteo_input(input_dir)
    df_PREC['PREC'] = df_PREC['PREC'].values
    df_pta = df_PREC.join(df_Ta)
    df_pta = df_pta.ffill()

    # event classification
    prec_event = event_classification(df_pta)
    ...
    # time steps (in hours)
    time_steps = np.ones((no_time_steps))
    time_steps[:] = events['dt'].values
    results['time_steps'] = time_steps
    # years
    years = np.zeros((no_time_steps), dtype=np.int32)
    years[:] = events.index.year.values
    results['years'] = years
    # months
    months = np.zeros((no_time_steps), dtype=np.int32)
    months[:] = events.index.month.values
    results['months'] = months
    # days of year
    days_of_year = np.zeros((no_time_steps), dtype=np.int32)
    days_of_year[:] = events.index.dayofyear.values
    results['days_of_year'] = days_of_year
    # hours
    hours = np.zeros((no_time_steps))
    hours[:] = events.index.hour.values
    results['hours'] = hours

    if (events['event_no'] > 0).any():
        _, frequency_events = np.unique(events['event_no'][events['event_no'] > 0], return_counts = True)
        # number of iterations of longest event
        nitt_event = np.max(frequency_events)
    ...
    if enable_film_flow:
        if (events['event_no_ff'] > 0).any():
            event_nos_ff = np.unique(events['event_no_ff'][events['event_no_ff'] > 0])
