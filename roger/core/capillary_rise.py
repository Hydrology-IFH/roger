...
def calc_cpr_rz(state):
    ...
    z = update(
        ...
        at[2:-2, 2:-2],
        (
            (vs.z_root[2:-2, 2:-2, vs.tau] + (vs.z_soil[2:-2, 2:-2] - vs.z_root[2:-2, 2:-2, vs.tau]) / 2)
            - vs.z_root[2:-2, 2:-2, vs.tau] / 2
        )
    ...
    vs.cpr_rz = update(
        vs.cpr_rz,
        at[2:-2, 2:-2],
        npx.where(npx.isnan(vs.cpr_rz[2:-2, 2:-2]), 0, vs.cpr_rz[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
    )
    vs.cpr_rz = update(
        vs.cpr_rz,
        at[2:-2, 2:-2],
        npx.where(vs.S_lp_rz[2:-2, 2:-2] > 0, 0, vs.cpr_rz[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
    )
    vs.cpr_rz = update(
        vs.cpr_rz,
        at[2:-2, 2:-2],
        npx.where(vs.h_rz[2:-2, 2:-2, vs.tau] > vs.h_ss[2:-2, 2:-2, vs.tau], 0, vs.cpr_rz[2:-2, 2:-2])
        * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    if settings.enable_film_flow:
        vs.cpr_rz = update(
            vs.cpr_rz,
            at[2:-2, 2:-2],
            npx.where(npx.sum(vs.S_f[2:-2, 2:-2, :], axis=-1) > 0, 0, vs.cpr_rz[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
        )
    ...
    vs.cpr_rz = update(
        ...
        at[2:-2, 2:-2],
        npx.where(
            (vs.cpr_rz[2:-2, 2:-2] > (vs.S_fp_ss[2:-2, 2:-2] + vs.S_lp_ss[2:-2, 2:-2])),
    ...
    vs.cpr_rz = update(
        ...
        at[2:-2, 2:-2],
        npx.where(
            (vs.cpr_rz[2:-2, 2:-2] > vs.S_ufc_rz[2:-2, 2:-2] - vs.S_fp_rz[2:-2, 2:-2])
            & (vs.S_ufc_rz[2:-2, 2:-2] - vs.S_fp_rz[2:-2, 2:-2] > 0),
            vs.S_ufc_rz[2:-2, 2:-2] - vs.S_fp_rz[2:-2, 2:-2],
            vs.cpr_rz[2:-2, 2:-2],
        )
        * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    mask1 = (
        ...
        & (vs.S_lp_ss <= 0)
        & (
            (vs.z_wf[:, :, vs.tau] < vs.z_root[:, :, vs.tau])
            | (vs.z_sat[:, :, vs.tau] < vs.z_soil[:, :] - vs.z_root[:, :, vs.tau])
        )
    )
    mask2 = (
        (vs.cpr_rz > 0)
        & (vs.S_lp_ss > 0)
        & (vs.cpr_rz <= vs.S_lp_ss)
        & (
            (vs.z_wf[:, :, vs.tau] < vs.z_root[:, :, vs.tau])
            | (vs.z_sat[:, :, vs.tau] < vs.z_soil[:, :] - vs.z_root[:, :, vs.tau])
        )
    ...
    vs.S_lp_ss = update(
        vs.S_lp_ss,
        at[2:-2, 2:-2],
        npx.where(mask3[2:-2, 2:-2], 0, vs.S_lp_ss[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    vs.S_fp_rz = update(
        vs.S_fp_rz,
        at[2:-2, 2:-2],
        npx.where(mask4[2:-2, 2:-2], vs.S_ufc_rz[2:-2, 2:-2], vs.S_fp_rz[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
    )

    return KernelOutput(
        cpr_rz=vs.cpr_rz, S_fp_rz=vs.S_fp_rz, S_lp_rz=vs.S_lp_rz, S_fp_ss=vs.S_fp_ss, S_lp_ss=vs.S_lp_ss
    )
...
def calc_cpr_ss(state):
    ...
    if settings.enable_groundwater_boundary | settings.enable_groundwater:
        ...
        vs.cpr_ss = update(
            vs.cpr_ss,
            at[2:-2, 2:-2],
            npx.where(npx.isnan(vs.cpr_ss[2:-2, 2:-2]), 0, vs.cpr_ss[2:-2, 2:-2]) * vs.maskCatch[2:-2, 2:-2],
        )
        vs.cpr_ss = update(
            vs.cpr_ss,
            at[2:-2, 2:-2],
        ...
        if settings.enable_film_flow:
            vs.cpr_ss = update(
                vs.cpr_ss,
                at[2:-2, 2:-2],
                npx.where(npx.sum(vs.S_f[2:-2, 2:-2, :], axis=-1) > 0, 0, vs.cpr_rz[2:-2, 2:-2])
                * vs.maskCatch[2:-2, 2:-2],
            )
        ...
        vs.S_fp_ss = update_add(
            vs.S_fp_ss,
            at[2:-2, 2:-2],
            vs.cpr_ss[2:-2, 2:-2] * vs.maskCatch[2:-2, 2:-2],
        )
...
@roger_kernel
def calc_capillary_rise_rz_transport_kernel(state):
    """
    ...
    vs.tt_cpr_rz = update(
        vs.tt_cpr_rz,
        at[2:-2, 2:-2, :],
        transport.calc_tt(state, vs.SA_ss, vs.sa_ss, vs.cpr_rz, vs.sas_params_cpr_rz)[2:-2, 2:-2, :]
    ...
    vs.TT_cpr_rz = update(
        vs.TT_cpr_rz,
        at[2:-2, 2:-2, 1:],
        npx.cumsum(vs.tt_cpr_rz[2:-2, 2:-2, :], axis=-1),
    )

    vs.sa_ss = update(
        vs.sa_ss,
        at[2:-2, 2:-2, :, :],
        transport.update_sa(state, vs.sa_ss, vs.tt_cpr_rz, vs.cpr_rz)[2:-2, 2:-2, :, :]
        * vs.maskCatch[2:-2, 2:-2, npx.newaxis, npx.newaxis],
    )
    ...
    vs.sa_rz = update_add(
        vs.sa_rz,
        at[2:-2, 2:-2, vs.tau, :],
        vs.tt_cpr_rz[2:-2, 2:-2, :] * vs.cpr_rz[2:-2, 2:-2, npx.newaxis] * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
...
@roger_kernel
def calc_capillary_rise_rz_transport_iso_kernel(state):
    """
    ...
    vs.TT_cpr_rz = update(
        vs.TT_cpr_rz,
        at[2:-2, 2:-2, 1:],
        npx.cumsum(vs.tt_cpr_rz[2:-2, 2:-2, :], axis=2),
    )
    ...
    vs.mtt_cpr_rz = update(
        vs.mtt_cpr_rz,
        at[2:-2, 2:-2, :],
        transport.calc_mtt(state, vs.sa_ss, vs.tt_cpr_rz, vs.cpr_rz, vs.msa_ss, alpha)[2:-2, 2:-2, :]
        * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
    vs.C_cpr_rz = update(
        vs.C_cpr_rz,
        at[2:-2, 2:-2],
        transport.calc_conc_iso_flux(state, vs.mtt_cpr_rz, vs.tt_cpr_rz, vs.cpr_rz)[2:-2, 2:-2]
        * vs.maskCatch[2:-2, 2:-2],
    )
    vs.C_iso_cpr_rz = update(
        vs.C_iso_cpr_rz,
        at[2:-2, 2:-2],
        transport.conc_to_delta(state, vs.C_cpr_rz)[2:-2, 2:-2] * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    vs.msa_rz = update(
        ...
        at[2:-2, 2:-2, vs.tau, :],
        npx.where(
            vs.tt_cpr_rz[2:-2, 2:-2, :] * vs.cpr_rz[2:-2, 2:-2, npx.newaxis] + vs.sa_rz[2:-2, 2:-2, vs.tau, :] > 0,
            vs.msa_rz[2:-2, 2:-2, vs.tau, :]
            * (
                vs.sa_rz[2:-2, 2:-2, vs.tau, :]
                / (vs.tt_cpr_rz[2:-2, 2:-2, :] * vs.cpr_rz[2:-2, 2:-2, npx.newaxis] + vs.sa_rz[2:-2, 2:-2, vs.tau, :])
            )
            + vs.mtt_cpr_rz[2:-2, 2:-2, :]
            * (
    ...
    return KernelOutput(
        sa_ss=vs.sa_ss,
        msa_ss=vs.msa_ss,
        tt_cpr_rz=vs.tt_cpr_rz,
        TT_cpr_rz=vs.TT_cpr_rz,
        mtt_cpr_rz=vs.mtt_cpr_rz,
        C_cpr_rz=vs.C_cpr_rz,
        C_iso_cpr_rz=vs.C_iso_cpr_rz,
        sa_rz=vs.sa_rz,
        msa_rz=vs.msa_rz,
    )
...
@roger_kernel
def calc_capillary_rise_rz_transport_anion_kernel(state):
    """
    ...
    vs.mtt_cpr_rz = update(
        vs.mtt_cpr_rz,
        at[2:-2, 2:-2, :],
        transport.calc_mtt(state, vs.sa_ss, vs.tt_cpr_rz, vs.cpr_rz, vs.msa_ss, vs.alpha_q)[2:-2, 2:-2, :]
        * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
    ...
    vs.C_cpr_rz = update(
        vs.C_cpr_rz,
        at[2:-2, 2:-2],
        npx.where(vs.cpr_rz[2:-2, 2:-2] > 0, npx.sum(vs.mtt_cpr_rz[2:-2, 2:-2, :], axis=-1) / vs.cpr_rz[2:-2, 2:-2], 0)
        * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    vs.M_cpr_rz = update(
        vs.M_cpr_rz,
        at[2:-2, 2:-2],
        npx.sum(vs.mtt_cpr_rz[2:-2, 2:-2, :], axis=-1) * vs.maskCatch[2:-2, 2:-2],
    )
    ...
    vs.msa_ss = update_add(
        vs.msa_ss,
        at[2:-2, 2:-2, vs.tau, :],
        -vs.mtt_cpr_rz[2:-2, 2:-2, :] * vs.maskCatch[2:-2, 2:-2, npx.newaxis],
    )
...
def calculate_capillary_rise_rz_transport(state):
    ...
    if settings.enable_offline_transport and not (
        ...
        vs.update(calc_capillary_rise_rz_transport_kernel(state))

    if settings.enable_offline_transport and (settings.enable_oxygen18 | settings.enable_deuterium):
        vs.update(calc_capillary_rise_rz_transport_iso_kernel(state))
    ...
    if settings.enable_offline_transport and (
        ...
        vs.update(calc_capillary_rise_rz_transport_anion_kernel(state))
