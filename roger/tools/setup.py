...
@roger_sync
def write_forcing_tracer(input_dir, tracer, nrows=1, ncols=1, uniform=True, float_type="float32"):
...
@roger_sync
def write_crop_rotation(input_dir, nrows=1, ncols=1, float_type="float32"):
...
@roger_sync
def write_forcing_event(input_dir, nrows=1, ncols=1, uniform=True, prec_correction=False, float_type="float32"):
...
def precipitation_correction(prec, ta, month, horizontal_shielding="b1"):
    ...
    LUT_PREC_CORR = pd.DataFrame(index=["summer", "winter", "mixed", "snow"], columns=["eps", "b1", "b2", "b3", "b4"])
...
def write_forcing_distributed(
    ...
    station_ids = [int(item) for item in os.listdir(input_dir) if item != ".DS_Store"]
    ...
    for i in station_ids:
        ...
        while _lock:
            try:
                df_PREC, df_PET, df_TA, df_RS = read_meteo(input_dir / f"{i}")
                _lock = False
                break
            except BlockingIOError:
                _lock = True
